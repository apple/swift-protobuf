...
extension Google_Protobuf_Value: ExpressibleByNilLiteral {
      ...
      public init(nilLiteral: ()) { self.init(kind: .nullValue(.nullValue)) }
}
...
extension Google_Protobuf_Value {
      ...
      /// given boolean value.
      public init(boolValue: Bool) { self.init(kind: .boolValue(boolValue)) }
      ...
      internal func serializeJSONValue(
            to encoder: inout JSONEncoder, options: JSONEncodingOptions
      ) throws {
            switch kind {
            ...
            case .structValue(let v)?:
                  encoder.append(text: try v.jsonString(options: options))
