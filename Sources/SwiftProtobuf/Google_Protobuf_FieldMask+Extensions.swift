...
private func parseJSONFieldNames(names: String) -> [String]? {
      ...
      for c in names {
            switch c {
            case ",":
                  if fieldNameCount == 0 { return nil }
      ...
      if fieldNameCount == 0 {  // Last field name can't be empty
            return nil }
      if let pbName = JSONToProto(name: fieldName) { split.append(pbName) }
      else { return nil }
      return split
}
...
extension Google_Protobuf_FieldMask {
      ...
      /// - Parameter protoPaths: The paths from which to create the field mask,
      ///   defined using the .proto names for the fields.
      public init(protoPaths: String...) { self.init(protoPaths: protoPaths) }
...
extension Google_Protobuf_FieldMask: _CustomJSONCodable {
      mutating func decodeJSON(from decoder: inout JSONDecoder) throws {
            ...
            if let names = parseJSONFieldNames(names: s) { paths = names }
            else { throw JSONDecodingError.malformedFieldMask }
      }
      ...
      func encodedJSONString(options: JSONEncodingOptions) throws -> String {
            ...
            for p in paths {
                  ...
                  else { throw JSONEncodingError.fieldMaskConversion }
            }
