      ...
      CustomDebugStringConvertible
{
      ...
      internal var fields = [Int: [AnyMessageExtension]]()

      public init() {}

      public init(arrayLiteral: Element...) { insert(contentsOf: arrayLiteral) }

      public init(_ others: SimpleExtensionMap...) {
            for other in others { formUnion(other) }
      }

      public subscript(messageType: Message.Type, fieldNumber: Int)
            -> AnyMessageExtension?
      {
            if let l = fields[fieldNumber] {
                  for e in l { if messageType == e.messageType { return e } }
            }
            return nil
      }

      public func fieldNumberForProto(
            messageType: Message.Type, protoFieldName: String
      ) -> Int? {
            // TODO: Make this faster...
            for (_, list) in fields {
                  for e in list {
                        if e.fieldName == protoFieldName && e.messageType
                              == messageType
      ...
      public mutating func insert(_ newValue: Element) {
            ...
            else { fields[fieldNumber] = [newValue] }
      }

      public mutating func insert(contentsOf: [Element]) {
            for e in contentsOf { insert(e) }
      }
            ...
            for (fieldNumber, otherList) in other.fields {
                  if let list = fields[fieldNumber] {
                        var newList = list.filter {
                              for o in otherList {
                                    if $0.messageType == o.messageType {
                                          return false
                                    }
                              }
                              return true
                  ...
                  else { fields[fieldNumber] = otherList }
            }
      }
