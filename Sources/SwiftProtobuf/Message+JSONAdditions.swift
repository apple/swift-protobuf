...
extension Message {
      ...
      public func jsonUTF8Data(
            ...
            var visitor = try JSONEncodingVisitor(
                  message: self, options: options)
            visitor.startObject()
      ...
      public init(
            ...
            if jsonString.isEmpty { throw JSONDecodingError.truncated }
            if let data = jsonString.data(using: String.Encoding.utf8) {
                  try self.init(jsonUTF8Data: data, options: options)
            }
            else { throw JSONDecodingError.truncated }
      }
      ...
      public init(
            ...
            try jsonUTF8Data.withUnsafeBytes { (bytes: UnsafePointer<UInt8>) in
                  let buffer = UnsafeBufferPointer(
                        start: bytes, count: jsonUTF8Data.count)
                  ...
                  if !decoder.scanner.skipOptionalNull() {
                        ...
                        if let message = try (
                              Self.self as! _CustomJSONCodable.Type
                        ).decodedFromJSONNull() { self = message as! Self }
                        else { throw JSONDecodingError.illegalNull }
                  }
