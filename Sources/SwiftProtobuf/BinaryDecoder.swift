...
internal struct BinaryDecoder: Decoder {
  ...
  internal mutating func decodeSingularEnumField<E: Enum>(value: inout E?) throws
  ...
  internal mutating func decodeRepeatedEnumField<E: Enum>(value: inout [E]) throws
  where E.RawValue == Int {
    switch fieldWireFormat {
    case WireFormat.varint:
      let varint = try decodeVarint()
    ...
    case WireFormat.lengthDelimited:
      ...
      if let extras = extras {
        ...
        let fieldSize =
          Varint.encodedSize(of: fieldTag.rawValue) + Varint.encodedSize(of: Int64(bodySize))
          + bodySize
  ...
  internal mutating func decodeExtensionFieldsAsMessageSet(
    ...
    while let fieldNumber = try self.nextFieldNumber() {
      guard
        fieldNumber == WireFormat.MessageSet.FieldNumbers.item
