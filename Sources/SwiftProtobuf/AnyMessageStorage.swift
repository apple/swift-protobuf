...
fileprivate func serializeAnyJSON(
      for message: Message, typeURL: String, options: JSONEncodingOptions
) throws -> String {
      ...
      else { try message.traverse(visitor: &visitor) }
      visitor.endObject()
      return visitor.stringResult
}

fileprivate func emitVerboseTextForm(
      visitor: inout TextFormatEncodingVisitor, message: Message,
      typeURL: String
) {
      let url: String
      if typeURL.isEmpty {
            url = buildTypeURL(
                  forMessage: message, typePrefix: defaultAnyTypeURLPrefix)
      }
      else { url = typeURL }
...
fileprivate func unpack(
      contentJSON: Data, options: JSONDecodingOptions,
      ...
      guard messageType is _CustomJSONCodable.Type else {
            ...
            return try messageType.init(
                  jsonUTF8Data: contentJSONAsObject, options: options)
      }

      var value = String()
      try contentJSON.withUnsafeBytes { (bytes: UnsafePointer<UInt8>) in
            let buffer = UnsafeBufferPointer(
                  start: bytes, count: contentJSON.count)
            var scanner = JSONScanner(
                  source: buffer, messageDepthLimit: options.messageDepthLimit,
...
internal class AnyMessageStorage {
      ...
      var _value: Data {
            ...
            get {
                  ...
                  case .message(let message):
                        do { return try message.serializedData(partial: true) }
                        catch { return Internal.emptyData }
                  case .contentJSON(let contentJSON, let options):
                        guard
                              let messageType = Google_Protobuf_Any.messageType(
                              forTypeURL: _typeURL)
                        else { return Internal.emptyData }
                        do {
                              let m = try unpack(
                                    contentJSON: contentJSON, options: options,
                                    as: messageType)
      ...
      func isA<M: Message>(_ type: M.Type) -> Bool {
            if _typeURL.isEmpty { return false }
      ...
      func unpackTo<M: Message>(
            target: inout M, extensions: ExtensionMap?,
            options: BinaryDecodingOptions
      ) throws {
            guard isA(M.self) else { throw AnyUnpackError.typeMismatch }

            switch state {
            case .binary(let data):
                  target = try M(
                        serializedData: data, extensions: extensions,
            ...
            case .contentJSON(let contentJSON, let options):
                  target = try unpack(
                        contentJSON: contentJSON, options: options, as: M.self)
                        as! M
            }
      }
      ...
      func preTraverse() throws {
            ...
            case .contentJSON:
                  ...
                  if Google_Protobuf_Any.messageType(forTypeURL: _typeURL)
                        == nil
                  {
...
extension AnyMessageStorage {
      func decodeTextFormat(
            typeURL url: String, decoder: inout TextFormatDecoder
      ) throws {
            ...
            var subDecoder = try TextFormatDecoder(
                  messageType: messageType, scanner: decoder.scanner,
      ...
      internal func textTraverse(visitor: inout TextFormatEncodingVisitor) {
            ...
            case .binary(let valueData):
                  if let messageType = Google_Protobuf_Any.messageType(
                        forTypeURL: _typeURL)
                  {
                        // If we can decode it, we can write the readable verbose form:
                        do {
                              let m = try messageType.init(
                                    serializedData: valueData, partial: true)
                              emitVerboseTextForm(
                                    visitor: &visitor, message: m,
                                    typeURL: _typeURL)
                              return
                        }
                        catch {
                  ...
                  if !_typeURL.isEmpty {
                        try! visitor.visitSingularStringField(
                              value: _typeURL, fieldNumber: 1)
                  }
                  if !valueData.isEmpty {
                        try! visitor.visitSingularBytesField(
                              value: valueData, fieldNumber: 2)
                  }

            case .message(let msg):
                  emitVerboseTextForm(
                        visitor: &visitor, message: msg, typeURL: _typeURL)
...
extension AnyMessageStorage {
      ...
      func isEqualTo(other: AnyMessageStorage) -> Bool {
            if _typeURL != other._typeURL { return false }
            ...
            if case .message(let myMsg) = state,
                  case .message(let otherMsg) = other.state, type(of: myMsg)
                  == type(of: otherMsg) {
            ...
            if case .binary(let myValue) = state,
                  case .binary(let otherValue) = other.state, myValue
                  == otherValue
            { return true }
            ...
            if case .contentJSON(let myJSON, _) = state,
                  case .contentJSON(let otherJSON, _) = other.state, myJSON
                  == otherJSON
            { return true }
...
extension AnyMessageStorage {
      ...
      func encodedJSONString(options: JSONEncodingOptions) throws -> String {
            ...
            case .binary(let valueData):
                  ...
                  let m = try messageType.init(
                        serializedData: valueData, partial: true)
                  return try serializeAnyJSON(
                        for: m, typeURL: _typeURL, options: options)

            case .message(let msg):
                  // We should have been initialized with a typeURL, but
                  // ensure it wasn't cleared.
                  let url = !_typeURL.isEmpty
                        ? _typeURL : buildTypeURL(
                              forMessage: msg,
                              typePrefix: defaultAnyTypeURLPrefix)
                  return try serializeAnyJSON(
                        for: msg, typeURL: url, options: options)
      ...
      func decodeJSON(from decoder: inout JSONDecoder) throws {
            ...
            if decoder.scanner.skipOptionalObjectEnd() { return }
            ...
            while true {
                  ...
                  if decoder.scanner.skipOptionalObjectEnd() {
                        ...
                        updatedOptions.messageDepthLimit
                              = decoder.scanner.recursionBudget
                        state = .contentJSON(
                              jsonEncoder.dataResult, updatedOptions)
                        return
                  }
