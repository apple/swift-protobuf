...
///
/// The decoder uses an ``ExtensionMap`` during decoding to look up
...
public protocol ExtensionMap: Sendable {
    ...
    ///
    /// The field name here matches the format protobuf Text serialization
    /// uses: it typically looks like
    /// `package.message.field_name`, where `package` is the package
    /// for the proto file and `message` is the name of the message in
    /// which the .proto file declares the extension. (This is different
    /// from the message that the extension actually extends!)
