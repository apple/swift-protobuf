...
extension MessageStorage {
    ...
    private func decodeNextField(
        ...
        guard let field = schema[fieldNumber: UInt32(tag.fieldNumber)] else {
            // Try to decode as an extension if the field doesn't exist in the message itself.
            // We also do this if we're parsing a message set; in that case, some other serializer
            // may have not known about the message set wire format and coded it as a regular
            // extension, and we need to support that.
        ...
        case .scalar:
            ...
            case .sint32:
                ...
                // If the number on the wire is larger than fits into an `Int32`, this is not an
                // error; we truncate it.
                updateValue(of: field, to: Int32(zigZagDecoded: UInt32(truncatingIfNeeded: try reader.nextVarint())))

            case .sint64:
                guard tag.wireFormat == .varint else { return false }
                updateValue(of: field, to: Int64(zigZagDecoded: try reader.nextVarint()))
