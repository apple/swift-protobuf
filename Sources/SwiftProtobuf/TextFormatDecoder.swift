...
internal struct TextFormatDecoder: Decoder {
      ...
      internal var complete: Bool { mutating get { return scanner.complete } }

      internal init(
            messageType: Message.Type, utf8Pointer: UnsafePointer<UInt8>,
            count: Int, extensions: ExtensionMap?
      ) throws {
            scanner = TextFormatScanner(
                  utf8Pointer: utf8Pointer, count: count, extensions: extensions
            )
            guard let nameProviding = (messageType as? _ProtoNameProviding.Type)
            else { throw TextFormatDecodingError.missingFieldNames }
            fieldNameMap = nameProviding._protobuf_nameMap
            self.messageType = messageType
      }

      internal init(
            messageType: Message.Type, scanner: TextFormatScanner,
            terminator: UInt8?
      ) throws {
      ...
      mutating func nextFieldNumber() throws -> Int? {
            if let terminator = terminator {
                  if scanner.skipOptionalObjectEnd(terminator) { return nil }
            }
            if fieldCount > 0 { scanner.skipOptionalSeparator() }
            if let key = try scanner.nextOptionalExtensionKey() {
                  // Extension key; look up in the extension registry
                  if let fieldNumber = scanner.extensions?.fieldNumberForProto(
                        messageType: messageType!, protoFieldName: key)
                  {
                        fieldCount += 1
                        return fieldNumber
                  }
            ...
            else if let fieldNumber = try scanner.nextFieldNumber(
                  names: fieldNameMap!)
            {
                  fieldCount += 1
                  return fieldNumber
            }
            else if terminator == nil { return nil }
            else { throw TextFormatDecodingError.truncated }
      ...
      mutating func decodeRepeatedFloatField(value: inout [Float]) throws {
            ...
            if scanner.skipOptionalBeginArray() {
                  var firstItem = true
                  while true {
                        if scanner.skipOptionalEndArray() { return }
      ...
      mutating func decodeSingularMessageField<M: Message>(value: inout M?)
            throws
      {
            ...
            var subDecoder = try TextFormatDecoder(
                  messageType: M.self, scanner: scanner, terminator: terminator)
            if M.self == Google_Protobuf_Any.self {
                  var any = value as! Google_Protobuf_Any?
                  try any!.decodeTextFormat(decoder: &subDecoder)
                  value = any as! M?
            }
            else { try value!.decodeMessage(decoder: &subDecoder) }
            scanner = subDecoder.scanner
      }

      mutating func decodeRepeatedMessageField<M: Message>(value: inout [M])
            throws
      {
            _ = scanner.skipOptionalColon()
            if scanner.skipOptionalBeginArray() {
                  ...
                  while true {
                        ...
                        let terminator = try scanner.skipObjectStart()
                        var subDecoder = try TextFormatDecoder(
                              messageType: M.self, scanner: scanner,
      ...
      mutating func decodeRepeatedGroupField<G: Message>(value: inout [G])
            throws
      ...
      private mutating func decodeMapEntry<KeyType, ValueType: MapValueType>(
            ...
            while true {
                  ...
                  if let key = try scanner.nextKey() {
                        switch key {
                        case "key", "1":
                              try KeyType.decodeSingular(
                                    value: &keyField, from: &self)
                        case "value", "2":
                              try ValueType.decodeSingular(
                                    value: &valueField, from: &self)
                        default: throw TextFormatDecodingError.unknownField
                        }
      ...
      mutating func decodeMapField<KeyType, ValueType: MapValueType>(
            ...
            else { try decodeMapEntry(mapType: fieldType, value: &value) }
      }
