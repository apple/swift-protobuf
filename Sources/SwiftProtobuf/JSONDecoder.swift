...
internal struct JSONDecoder: Decoder {
      ...
      internal init(
            source: UnsafeBufferPointer<UInt8>, options: JSONDecodingOptions
      ) {
      ...
      mutating func nextFieldNumber() throws -> Int? {
            if scanner.skipOptionalObjectEnd() { return nil }
            if fieldCount > 0 { try scanner.skipRequiredComma() }
      ...
      mutating func decodeRepeatedFloatField(value: inout [Float]) throws {
            if scanner.skipOptionalNull() { return }
            try scanner.skipRequiredArrayStart()
            if scanner.skipOptionalArrayEnd() { return }
            while true {
      ...
      mutating func decodeSingularUInt32Field(value: inout UInt32) throws {
            ...
            if n > UInt64(UInt32.max) { throw JSONDecodingError.numberRange }
      ...
      mutating func decodeSingularBoolField(value: inout Bool) throws {
            ...
            if isMapKey { value = try scanner.nextQuotedBool() }
            else { value = try scanner.nextBool() }
      }
      ...
      internal mutating func decodeFullObject<M: Message>(message: inout M)
            throws
      {
            guard let nameProviding = (M.self as? _ProtoNameProviding.Type)
            else { throw JSONDecodingError.missingFieldNames }
            ...
            if let m = message as? _CustomJSONCodable {
                  ...
                  if scanner.skipOptionalObjectEnd() { return }
            ...
            throws
      {
            if scanner.skipOptionalNull() {
                  if M.self is _CustomJSONCodable.Type {
                        value = try (M.self as! _CustomJSONCodable.Type
            ...
            assert(
                  scanner.recursionBudget == subDecoder.scanner.recursionBudget)
            ...
            throws
      {
            ...
            while true {
                  if scanner.skipOptionalNull() {
                        ...
                        if M.self is _CustomJSONCodable.Type {
                              if let message = try (
                                    M.self as! _CustomJSONCodable.Type
                              ).decodedFromJSONNull() as? M {
                                    value.append(message)
                                    appended = true
                              }
                        }
                        if !appended { throw JSONDecodingError.illegalNull }
                  }
                  else {
                        var message = M()
                        var subDecoder = JSONDecoder(decoder: self)
                        ...
                        value.append(message)
                        assert(
                              scanner.recursionBudget
                                    == subDecoder.scanner.recursionBudget)
                        scanner = subDecoder.scanner
                  }
                  if scanner.skipOptionalArrayEnd() { return }
                  try scanner.skipRequiredComma()
      ...
      mutating func decodeMapField<KeyType, ValueType: MapValueType>(
            ...
            while true {
                  ...
                  if c != "\"" { throw JSONDecodingError.unquotedMapKey }
                  isMapKey = true
                  ...
                  else { throw JSONDecodingError.malformedMap }
                  if scanner.skipOptionalObjectEnd() { return }
                  try scanner.skipRequiredComma()
            }
      }
      ...
      mutating func decodeExtensionField(
            ...
            fieldNumber: Int
      ) throws { throw JSONDecodingError.schemaMismatch }
}
