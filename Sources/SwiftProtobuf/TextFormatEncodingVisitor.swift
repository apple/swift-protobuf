...
internal struct TextFormatEncodingVisitor: Visitor {
      ...
      var result: String { return encoder.stringResult }
      ...
      private init(message: Message, encoder: TextFormatEncoder) {
            ...
            let extensions = (message as? ExtensibleMessage
            )?._protobuf_extensionFieldValues
            self.init(
                  nameMap: nameMap, nameResolver: [:], extensions: extensions,
                  encoder: encoder)
      }

      private init(
            nameMap: _NameMap?, nameResolver: [Int: StaticString],
            extensions: ExtensionFieldValueSet?, encoder: TextFormatEncoder
      ) {
      ...
      private mutating func emitFieldName(lookingUp fieldNumber: Int) {
            ...
            else if let extensionName = extensions?[
                  fieldNumber]?.protobufExtension.fieldName
            { encoder.emitExtensionFieldName(name: extensionName) }
            else { encoder.emitFieldNumber(number: fieldNumber) }
      }

      mutating func visitUnknown(bytes: Data) throws {
            try bytes.withUnsafeBytes { (p: UnsafePointer<UInt8>) -> () in
                  var decoder = BinaryDecoder(
                        forReadingFrom: p, count: bytes.count,
      ...
      private mutating func visitUnknown(
            decoder: inout BinaryDecoder, groupFieldNumber: Int?
      ) throws {
            while let tag = try decoder.getTag() {
                  ...
                  case .startGroup:
                        ...
                        try visitUnknown(
                              decoder: &decoder,
                              groupFieldNumber: tag.fieldNumber)
                        encoder.endMessageField()
                  case .endGroup:
      ...
      mutating func visitSingularMessageField<M: Message>(
            ...
            emitFieldName(lookingUp: fieldNumber)
            encoder.startMessageField()
            var visitor = TextFormatEncodingVisitor(
                  message: value, encoder: encoder)
            if let any = value as? Google_Protobuf_Any {
                  any.textTraverse(visitor: &visitor)
            }
            else { try! value.traverse(visitor: &visitor) }
      ...
      mutating func visitRepeatedMessageField<M: Message>(
            ...
            for v in value {
                  ...
                  encoder.startMessageField()
                  var visitor = TextFormatEncodingVisitor(
                        message: v, encoder: encoder)
                  if let any = v as? Google_Protobuf_Any {
                        any.textTraverse(visitor: &visitor)
                  }
                  else { try! v.traverse(visitor: &visitor) }
                  encoder = visitor.encoder
                  encoder.endMessageField()
            }
      }
      ...
      private mutating func _visitPacked<T>(
            ...
            encoder.startArray()
            for v in value {
                  if !firstItem { encoder.arraySeparator() }
                  encode(v, &encoder)
                  firstItem = false
            }
            encoder.endArray()
      ...
      private mutating func _visitMap<K, V>(
            map: Dictionary<K, V>, fieldNumber: Int,
            ...
            for (k, v) in map {
                  ...
                  encoder.startMessageField()
                  var visitor = TextFormatEncodingVisitor(
                        nameMap: nil, nameResolver: mapNameResolver,
                        extensions: nil, encoder: encoder)
      ...
      mutating func visitMapField<KeyType, ValueType: MapValueType>(
            ...
            try _visitMap(map: value, fieldNumber: fieldNumber) {
                  (visitor: inout TextFormatEncodingVisitor, key, value) throws
                        -> () in
                  try KeyType.visitSingular(
                        value: key, fieldNumber: 1, with: &visitor)
                  try ValueType.visitSingular(
                        value: value, fieldNumber: 2, with: &visitor)
            }
      }
