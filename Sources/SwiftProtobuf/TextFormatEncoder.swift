...
internal struct TextFormatEncoder {
      ...
      internal mutating func append(staticText: StaticString) {
            let buff = UnsafeBufferPointer(
                  start: staticText.utf8Start,
                  count: staticText.utf8CodeUnitCount)
      ...
      internal mutating func indent() { data.append(contentsOf: indentString) }
      ...
      mutating func emitFieldName(name: StaticString) {
            let buff = UnsafeBufferPointer(
                  start: name.utf8Start, count: name.utf8CodeUnitCount)
      ...
      mutating func startRegularField() { append(staticText: ": ") }
      mutating func endRegularField() { data.append(asciiNewline) }
      ...
      mutating func startMessageField() {
            append(staticText: " {\n")
            for _ in 1...tabSize { indentString.append(asciiSpace) }
      }
      ...
      mutating func startArray() { data.append(asciiOpenSquareBracket) }

      mutating func arraySeparator() { append(staticText: ", ") }

      mutating func endArray() { data.append(asciiCloseSquareBracket) }

      mutating func putEnumValue<E: Enum>(value: E) {
            if let name = value.name {
                  data.append(contentsOf: name.utf8Buffer)
            }
            else { appendInt(value: Int64(value.rawValue)) }
      }

      mutating func putFloatValue(value: Float) {
            if value.isNaN { append(staticText: "nan") }
            else if !value.isFinite {
                  if value < 0 { append(staticText: "-inf") }
                  else { append(staticText: "inf") }
            }
            else {
      ...
      mutating func putDoubleValue(value: Double) {
            if value.isNaN { append(staticText: "nan") }
                  ...
                  if let v = Int64(exactly: value) { appendInt(value: v) }
                  else {
                        let doubleFormatter = DoubleFormatter()
      ...
      private mutating func appendUInt(value: UInt64) {
            if value >= 1000 { appendUInt(value: value / 1000) }
            if value >= 100 {
                  data.append(asciiZero + UInt8((value / 100) % 10))
            }
            if value >= 10 { data.append(asciiZero + UInt8((value / 10) % 10)) }
      ...
      private mutating func appendInt(value: Int64) {
            ...
            else { appendUInt(value: UInt64(bitPattern: value)) }
      }

      mutating func putInt64(value: Int64) { appendInt(value: value) }

      mutating func putUInt64(value: UInt64) { appendUInt(value: value) }

      mutating func appendUIntHex(value: UInt64, digits: Int) {
            if digits == 0 { append(staticText: "0x") }
            else {
      ...
      mutating func putStringValue(value: String) {
            ...
            for c in value.unicodeScalars {
                  ...
                  case 0x800...0xffff:
                        data.append(
                              0xe0 + UInt8(truncatingIfNeeded: c.value >> 12))
                        data.append(
                              0x80 + UInt8(
                                    truncatingIfNeeded: (c.value >> 6) & 0x3f))
                        data.append(
                              0x80 + UInt8(truncatingIfNeeded: c.value & 0x3f))
                  default:
                        data.append(
                              0xf0 + UInt8(truncatingIfNeeded: c.value >> 18))
                        data.append(
                              0x80 + UInt8(
                                    truncatingIfNeeded: (c.value >> 12) & 0x3f))
                        data.append(
                              0x80 + UInt8(
                                    truncatingIfNeeded: (c.value >> 6) & 0x3f))
                        data.append(
      ...
      mutating func putBytesValue(value: Data) {
            ...
            value.withUnsafeBytes { (p: UnsafePointer<UInt8>) in
                  for i in 0..<value.count {
                        let c = p[i]
                        switch c {// Special two-byte escapes
                        case 8: append(staticText: "\\b")
                        case 9: append(staticText: "\\t")
                        case 10: append(staticText: "\\n")
                        case 11: append(staticText: "\\v")
