...
internal func buildTypeURL(forMessage message: Message, typePrefix: String)
      -> String
{
      var url = typePrefix
...
extension Google_Protobuf_Any {
      ...
      @discardableResult public static func register(messageType: Message.Type)
            -> Bool
      {
      ...
      public static func messageType(forMessageName name: String)
            -> Message.Type?
      {
            var result: Message.Type?
            serialQueue.sync { result = knownTypes[name] }
            return result
      }
