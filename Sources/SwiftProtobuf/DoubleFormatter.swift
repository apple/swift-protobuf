...
private func wrapped_vsnprintf(
      destination: UnsafeMutableRawBufferPointer,
      format: UnsafeMutableRawBufferPointer, _ arguments: CVarArg...
) -> Int {
...
internal class DoubleFormatter {
      ...
      init() {
            ...
            let formatBytes = UnsafeBufferPointer(
                  start: format.utf8Start, count: format.utf8CodeUnitCount)
            #if swift(>=4.1)
                  doubleFormatString = UnsafeMutableRawBufferPointer.allocate(
                        byteCount: formatBytes.count + 1,
                        alignment: MemoryLayout<UInt8>.alignment)
            #else
                  doubleFormatString = UnsafeMutableRawBufferPointer.allocate(
                        count: formatBytes.count + 1)
            #endif
      ...
      func utf8ToDouble(bytes: UnsafePointer<UInt8>, count: Int) -> Double? {
            ...
            if e != start + count || !d.isFinite { return nil }
            return d
      }
      ...
      private func _doubleToUtf8(_ d: Double, digits: Int32)
            -> UnsafeBufferPointer<UInt8> {
            // Format into the work buffer, return a UBP with the result
            let count = wrapped_vsnprintf(
                  destination: work, format: doubleFormatString, digits, d)
