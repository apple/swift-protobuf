...
public struct ExtensionFieldValueSet: Hashable {
      ...
      public static func == (
            lhs: ExtensionFieldValueSet, rhs: ExtensionFieldValueSet
      ) -> Bool {
            guard lhs.values.count == rhs.values.count else { return false }
            for (index, l) in lhs.values {
                  if let r = rhs.values[index] {
                        if type(of: l) != type(of: r) { return false }
                        if !l.isEqual(other: r) { return false }
                  }
                  else { return false }
            }
            return true
      ...
      public func traverse<V: Visitor>(visitor: inout V, start: Int, end: Int)
            throws
      {
      ...
      public var isInitialized: Bool {
            for (_, v) in values { if !v.isInitialized { return false } }
            return true
      }
}
