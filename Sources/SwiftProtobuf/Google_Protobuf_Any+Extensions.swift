...
extension Google_Protobuf_Any {
      ...
      public init(
            message: Message, partial: Bool = false,
      ...
      public init(textFormatString: String, extensions: ExtensionMap? = nil)
            throws
      {
            self.init()
            if !textFormatString.isEmpty {
                  if let data = textFormatString.data(
                        using: String.Encoding.utf8)
                  {
                        try data.withUnsafeBytes {
                              ...
                              var textDecoder = try TextFormatDecoder(
                                    ...
                                    utf8Pointer: bytes, count: data.count,
                                    extensions: extensions)
                              try decodeTextFormat(decoder: &textDecoder)
                              if !textDecoder.complete {
                                    throw
                                          TextFormatDecodingError.trailingGarbage
                              }
                        }
                  }
            }
...
extension Google_Protobuf_Any: _CustomJSONCodable {
      ...
      internal mutating func decodeTextFormat(decoder: inout TextFormatDecoder)
            throws
      {
            ...
            if let url = try decoder.scanner.nextOptionalAnyURL() {
                  try _uniqueStorage().decodeTextFormat(
                        typeURL: url, decoder: &decoder)
            }
            else {
      ...
      internal func encodedJSONString(options: JSONEncodingOptions) throws
            -> String
