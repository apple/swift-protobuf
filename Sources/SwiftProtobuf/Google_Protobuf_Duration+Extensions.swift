...
private func parseDuration(text: String) throws -> (Int64, Int32) {
      ...
      while let c = chars.next() {
            ...
            case "-":
                  ...
                  if total > 0 { throw JSONDecodingError.malformedDuration }
                  digits.append(c)
            ...
            case ".":
                  ...
                  if let s = Int64(digitString), s >= minDurationSeconds && s
                        <= maxDurationSeconds
                  { seconds = s }
                  else { throw JSONDecodingError.malformedDuration }
                  digits.removeAll()
                  digitCount = 0
            case "s":
                  if let seconds = seconds {
                        ...
                        let digitString = String(digits)
                        if let s = Int64(digitString), s >= minDurationSeconds
            ...
            default: throw JSONDecodingError.malformedDuration
            }
            total += 1
      }
      throw JSONDecodingError.malformedDuration
...
private func formatDuration(seconds: Int64, nanos: Int32) -> String? {
      let (seconds, nanos) = normalizeForDuration(
            seconds: seconds, nanos: nanos)
      guard seconds >= minDurationSeconds && seconds <= maxDurationSeconds
      else { return nil }

      if nanos == 0 { return String(format: "%llds", seconds) }
      else if nanos % 1_000_000 == 0 {
            return String(format: "%lld.%03ds", seconds, abs(nanos) / 1_000_000)
      }
      else if nanos % 1000 == 0 {
            return String(format: "%lld.%06ds", seconds, abs(nanos) / 1000)
      }
      else { return String(format: "%lld.%09ds", seconds, abs(nanos)) }
}
...
extension Google_Protobuf_Duration: _CustomJSONCodable {
      ...
      func encodedJSONString(options: JSONEncodingOptions) throws -> String {
            ...
            else { throw JSONEncodingError.durationRange }
      }
}
...
private func normalizeForDuration(seconds: Int64, nanos: Int32)
...
public prefix func - (operand: Google_Protobuf_Duration)
      -> Google_Protobuf_Duration
{
      let (s, n) = normalizeForDuration(
            seconds: -operand.seconds, nanos: -operand.nanos)
      return Google_Protobuf_Duration(seconds: s, nanos: n)
}

public func + (lhs: Google_Protobuf_Duration, rhs: Google_Protobuf_Duration)
      -> Google_Protobuf_Duration
{
      let (s, n) = normalizeForDuration(
...
public func - (lhs: Google_Protobuf_Timestamp, rhs: Google_Protobuf_Timestamp)
      -> Google_Protobuf_Duration
{
      let (s, n) = normalizeForDuration(
            seconds: lhs.seconds - rhs.seconds, nanos: lhs.nanos - rhs.nanos)
