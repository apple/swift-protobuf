...
/// The errors that can occur when decoding a message from binary format.
public enum BinaryDecodingError: Error {
    ...
    /// fields that the binary data didn't include values for.
    ///
    /// Pass `partial: true` during decoding if you want to explicitly ignore
