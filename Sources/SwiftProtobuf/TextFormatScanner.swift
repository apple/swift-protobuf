...
internal struct TextFormatScanner {
  ...
  private mutating func validateAndCountBytesFromString(terminator: UInt8, sawBackslash: inout Bool)
    throws -> Int
  {
    var count = 0
    let start = p
    ...
    loop: while p != end {
      ...
      case asciiLowerF:  // f
        ...
        let d = doubleParser.utf8ToDouble(
          bytes: UnsafeRawBufferPointer(start: start, count: p - start))
        // Just skip the 'f'
        p += 1
        skipWhitespace()
        return d
  ...
  private mutating func skipOptionalNaN() -> Bool {
    return skipOptionalKeyword(bytes: [asciiLowerN, asciiLowerA, asciiLowerN])
  }
