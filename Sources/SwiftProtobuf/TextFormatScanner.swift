...
private func fromHexDigit(_ c: UInt8) -> UInt8? {
      if c >= asciiZero && c <= asciiNine { return c - asciiZero }
            ...
            case asciiBackslash:  // backslash
                  if let escaped = bytes.next() {
                        ...
                        case asciiZero...asciiSeven:  // 0...7
                              ...
                              if let digit2 = bytes.next(), digit2 >= asciiZero
                                    && digit2 <= asciiSeven
                              {
                                    let digit2Value = digit2 - asciiZero
                                    let innerSavedPosition = bytes
                                    if let digit3 = bytes.next(), digit3
                                          >= asciiZero && digit3 <= asciiSeven
                                    {
                                          let digit3Value = digit3 - asciiZero
                                          let n = digit1Value * 64 + digit2Value
                                                * 8 + digit3Value
                                          out.append(n)
                                    }
                                    else {
                        ...
                        case asciiLowerX:  // "x"
                              ...
                              if let byte = bytes.next(),
                                    let digit = fromHexDigit(byte) {
                                    var n = digit
                                    let savedPosition = bytes
                                    if let byte = bytes.next(),
                                          let digit = fromHexDigit(byte) {
                        ...
                        case asciiDoubleQuote, asciiSingleQuote,
                              asciiQuestionMark, asciiBackslash:  // " ' ? \
                              out.append(escaped)
                        default:
...
internal struct TextFormatScanner {
      ...
      internal var complete: Bool { mutating get { return p == end } }

      internal init(
            utf8Pointer: UnsafePointer<UInt8>, count: Int,
      ...
      private mutating func skipWhitespace() {
            while p != end {
                  let u = p[0]
                  switch u {
                  case asciiSpace, asciiTab, asciiNewLine,
      ...
      private mutating func parseUTF8Identifier() -> UnsafeBufferPointer<UInt8>
      {
            ...
            loop: while p != end {
                  ...
                  case asciiLowerA...asciiLowerZ, asciiUpperA...asciiUpperZ,
                        asciiZero...asciiNine, asciiUnderscore:
                        p += 1
                  default: break loop
                  }
            }
      ...
      private mutating func parseExtensionKey() -> String? {
            ...
            let c = p[0]
            switch c {
            case asciiLowerA...asciiLowerZ, asciiUpperA...asciiUpperZ: p += 1
            default: return nil
            }
            while p != end {
                  let c = p[0]
                  switch c {
                  case asciiLowerA...asciiLowerZ, asciiUpperA...asciiUpperZ,
                        asciiZero...asciiNine, asciiUnderscore, asciiPeriod,
                        asciiForwardSlash:
                        p += 1
      ...
      private mutating func validateAndCountBytesFromString(
            terminator: UInt8, sawBackslash: inout Bool
      ) throws -> Int {
            var count = 0
            let start = p
            sawBackslash = false
                        ...
                        if p != end {
                                          ...
                                          <= asciiSeven
                                    {
                                                ...
                                                <= asciiSeven
                                          {
                                                if escaped > asciiThree {
                                                      ...
                                                      throw
                                                            TextFormatDecodingError.malformedText
                                                }
                                                p += 1
                                          }
                                    }
                              ...
                              case asciiLowerX:  // 'x' hexadecimal escape
                                    if p != end && fromHexDigit(p[0]) != nil {
                                          p += 1
                                          if p != end && fromHexDigit(p[0])
                                                != nil
                                          { p += 1 }
                                    }
                                    else {
                                          throw
                                                TextFormatDecodingError.malformedText  // Hex escape must have at least 1 digit
                                    }
                                    count += 1
                              ...
                              default:
                                    throw
                                          TextFormatDecodingError.malformedText  // Unrecognized escape
                              }
                        }
                  default: count += 1
                  }
      ...
      private mutating func parseBytesFromString(
            terminator: UInt8, into data: inout Data
      ) {
                        ...
                        case asciiBackslash:  //  "\\"
                              ...
                              case asciiZero...asciiSeven:  // '0'...'7'
                                    ...
                                    if digit2 >= asciiZero, digit2 <= asciiSeven
                                    {
                                          ...
                                          let digit2Value = digit2 - asciiZero
                                          let digit3 = p[0]
                                          if digit3 >= asciiZero, digit3
                                                <= asciiSeven
                                          {
                                                p += 1
                                                let digit3Value = digit3
                                                      - asciiZero
                                                out[0] = digit1Value &* 64
                                                      + digit2Value * 8
                                                      + digit3Value
                                                out += 1
                                          }
      ...
      private mutating func parseStringSegment(terminator: UInt8) -> String? {
            ...
            while p != end {
                  ...
                  if c == terminator {
                        ...
                        p += 1
                        skipWhitespace()
                        if let s = s, sawBackslash { return decodeString(s) }
                        else { return s }
                  }
                  p += 1
                  if c == asciiBackslash {  //  \
      ...
      internal mutating func nextUInt() throws -> UInt64 {
            if p == end { throw TextFormatDecodingError.malformedNumber }
            let c = p[0]
            p += 1
            if c == asciiZero {  // leading '0' precedes octal or hex
                  if p[0] == asciiLowerX {  // 'x' => hex
                        ...
                        while p != end {
                              ...
                              if n > UInt64.max / 16 {
                                    throw
                                          TextFormatDecodingError.malformedNumber
                              }
                              p += 1
                              n = n * 16 + val
                        }
      ...
      internal mutating func nextStringValue() throws -> String {
            var result: String
            skipWhitespace()
            if p == end { throw TextFormatDecodingError.malformedText }
            let c = p[0]
            if c != asciiSingleQuote && c != asciiDoubleQuote {
                  throw TextFormatDecodingError.malformedText
            }
            p += 1
            if let s = parseStringSegment(terminator: c) { result = s }
            else { throw TextFormatDecodingError.malformedText }
      ...
      internal mutating func nextBytesValue() throws -> Data {
            ...
            p += 1
            var sawBackslash = false
            let n = try validateAndCountBytesFromString(
                  terminator: c, sawBackslash: &sawBackslash)
            if sawBackslash {
                  result = Data(count: n)
      ...
      private mutating func tryParseFloatString() -> Double? {
            ...
            loop: while p != end {
                  let c = p[0]
                  switch c {
                  case asciiZero...asciiNine, asciiPeriod, asciiPlus,
                        asciiMinus, asciiLowerE, asciiUpperE:  // 0...9, ., +, -, e, E
                        p += 1
                  case asciiLowerF:  // f
                        // proto1 allowed floats to be suffixed with 'f'
                        let d = doubleFormatter.utf8ToDouble(
                              bytes: start, count: p - start)
                        // Just skip the 'f'
                        p += 1
                        skipWhitespace()
                        return d
      ...
      private mutating func skipOptionalNaN() -> Bool {
            return skipOptionalKeyword(
                  bytes: [asciiLowerN, asciiLowerA, asciiLowerN])
      }
      ...
      private mutating func skipOptionalInfinity() -> Float? {
            ...
            if skipOptionalKeyword(bytes: inf) || skipOptionalKeyword(
                  bytes: infinity)
      ...
      internal mutating func nextFloat() throws -> Float {
            ...
            if skipOptionalNaN() { return Float.nan }
            if let inf = skipOptionalInfinity() { return inf }
            throw TextFormatDecodingError.malformedNumber
      }

      internal mutating func nextDouble() throws -> Double {
            if let d = tryParseFloatString() { return d }
            if skipOptionalNaN() { return Double.nan }
            if let inf = skipOptionalInfinity() { return Double(inf) }
            throw TextFormatDecodingError.malformedNumber
      }
      ...
      internal mutating func nextBool() throws -> Bool {
            ...
            let result: Bool
            switch c {
            case asciiZero: result = false
            case asciiOne: result = true
            case asciiLowerF, asciiUpperF:
                  if p != end {
                        let alse = [
                              asciiLowerA, asciiLowerL, asciiLowerS, asciiLowerE
                        ]
            ...
            case asciiLowerT, asciiUpperT:
                  ...
                  result = true
            default: throw TextFormatDecodingError.malformedText
            }
            if p == end { return result }
            switch p[0] {
            case asciiSpace, asciiTab, asciiNewLine, asciiCarriageReturn,
                  asciiHash, asciiComma, asciiSemicolon,
                  asciiCloseSquareBracket, asciiCloseCurlyBracket,
      ...
      internal mutating func nextOptionalEnumName() throws
      ...
      internal mutating func nextFieldNumber(names: _NameMap) throws -> Int? {
            ...
            let c = p[0]
            switch c {
            case asciiLowerA...asciiLowerZ, asciiUpperA...asciiUpperZ:  // a...z, A...Z
                  let key = parseUTF8Identifier()
                  if let fieldNumber = names.number(forProtoName: key) {
                        return fieldNumber
                  }
                  else { throw TextFormatDecodingError.unknownField }
            case asciiOne...asciiNine:  // 1-9 (field numbers are 123, not 0123)
                  ...
                  skipWhitespace()
                  if names.names(for: fieldNum) != nil { return fieldNum }
                  else {
