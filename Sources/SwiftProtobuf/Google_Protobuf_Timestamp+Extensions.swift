...
private func parseTimestamp(s: String) throws -> (Int64, Int32) {
      ...
      if value.count < 20 { throw JSONDecodingError.malformedTimestamp }
      ...
      func fromAscii4(
            _ digit0: Int, _ digit1: Int, _ digit2: Int, _ digit3: Int
      ) throws -> Int {
            if digit0 < zero || digit0 > nine || digit1 < zero || digit1 > nine
                  || digit2 < zero || digit2 > nine || digit3 < zero || digit3
                  > nine
            { throw JSONDecodingError.malformedTimestamp }
      ...
      if second > Int(61) { throw JSONDecodingError.malformedTimestamp }
      ...
      if value[pos] == plus || value[pos] == dash {
            ...
            if hourOffset > Int(13) || minuteOffset > Int(59) || value[pos + 3]
                  != colon
            { throw JSONDecodingError.malformedTimestamp }
      ...
      else { throw JSONDecodingError.malformedTimestamp }
      if pos != value.count { throw JSONDecodingError.malformedTimestamp }
      return (seconds, nanos)
}

private func formatTimestamp(seconds: Int64, nanos: Int32) -> String? {
      let (seconds, nanos) = normalizeForTimestamp(
            seconds: seconds, nanos: nanos)
      guard seconds >= minTimestampSeconds && seconds <= maxTimestampSeconds
      else { return nil }
      ...
      if nanos == 0 {
            return String(
                  format: "%04d-%02d-%02dT%02d:%02d:%02dZ", YY, MM, DD, hh, mm,
                  ss)
      }
      else if nanos % 1_000_000 == 0 {
            return String(
                  format: "%04d-%02d-%02dT%02d:%02d:%02d.%03dZ", YY, MM, DD, hh,
                  mm, ss, nanos / 1_000_000)
      }
      else if nanos % 1000 == 0 {
            return String(
                  format: "%04d-%02d-%02dT%02d:%02d:%02d.%06dZ", YY, MM, DD, hh,
                  mm, ss, nanos / 1000)
      }
      else {
            return String(
                  format: "%04d-%02d-%02dT%02d:%02d:%02d.%09dZ", YY, MM, DD, hh,
                  mm, ss, nanos)
      }
}
...
extension Google_Protobuf_Timestamp: _CustomJSONCodable {
      ...
      func encodedJSONString(options: JSONEncodingOptions) throws -> String {
            ...
            else { throw JSONEncodingError.timestampRange }
      }
}
...
extension Google_Protobuf_Timestamp {
      ...
      public init(timeIntervalSince1970: TimeInterval) {
            ...
            let nd = round(
                  (timeIntervalSince1970 - sd) * TimeInterval(nanosPerSecond))
            let (s, n) = normalizeForTimestamp(
                  seconds: Int64(sd), nanos: Int32(nd))
      ...
      public init(timeIntervalSinceReferenceDate: TimeInterval) {
            ...
            let nd = round(
                  (timeIntervalSinceReferenceDate - sd) * TimeInterval(
                        nanosPerSecond))
      ...
      public var timeIntervalSince1970: TimeInterval {
            return TimeInterval(self.seconds) + TimeInterval(self.nanos)
...
private func normalizeForTimestamp(seconds: Int64, nanos: Int32)
...
public func + (lhs: Google_Protobuf_Timestamp, rhs: Google_Protobuf_Duration)
      -> Google_Protobuf_Timestamp
{
      let (s, n) = normalizeForTimestamp(
            seconds: lhs.seconds + rhs.seconds, nanos: lhs.nanos + rhs.nanos)
      return Google_Protobuf_Timestamp(seconds: s, nanos: n)
}

public func + (lhs: Google_Protobuf_Duration, rhs: Google_Protobuf_Timestamp)
      -> Google_Protobuf_Timestamp
{
      let (s, n) = normalizeForTimestamp(
            seconds: lhs.seconds + rhs.seconds, nanos: lhs.nanos + rhs.nanos)
