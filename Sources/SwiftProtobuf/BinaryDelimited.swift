...
public enum BinaryDelimited {
      ...
      public static func serialize(
            message: Message, to stream: OutputStream, partial: Bool = false
      ) throws {
            ...
            let totalSize = Varint.encodedSize(of: UInt64(serialized.count))
                  + serialized.count
      ...
      public static func parse<M: Message>(
            messageType: M.Type, from stream: InputStream,
            extensions: ExtensionMap? = nil, partial: Bool = false,
            options: BinaryDecodingOptions = BinaryDecodingOptions()
      ) throws -> M {
            var message = M()
            try merge(
                  into: &message, from: stream, extensions: extensions,
      ...
      public static func merge<M: Message>(
            into message: inout M, from stream: InputStream,
            extensions: ExtensionMap? = nil, partial: Bool = false,
            options: BinaryDecodingOptions = BinaryDecodingOptions()
      ) throws {
...
internal func decodeVarint(_ stream: InputStream) throws -> UInt64 {
      ...
      while true {
            ...
            shift += 7
            if shift > 63 { throw BinaryDecodingError.malformedProtobuf }
      }
}
