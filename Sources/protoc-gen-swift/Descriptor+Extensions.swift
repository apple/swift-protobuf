...
extension FileDescriptor {
      ...
      var hasUnknownEnumPreservingSemantics: Bool { return syntax == .proto3 }

      /// True of primative field types should have field presence.
      var hasPrimativeFieldPresence: Bool { return syntax == .proto2 }
...
extension Descriptor {
      ...
      func hasRequiredFields() -> Bool {
            ...
            func hasRequiredFieldsInner(_ descriptor: Descriptor) -> Bool {
                  ...
                  if !descriptor.extensionRanges.isEmpty { return true }

                  for f in descriptor.fields {
                        if f.label == .required { return true }
                        switch f.type {
                        case .group, .message:
      ...
      var swiftExtensionRangeExpressions: String {
            return extensionRanges.lazy.map { $0.swiftRangeExpression }.joined(
                  separator: ", ")
      }
...
extension FieldDescriptor {
      ...
      func swiftType(namer: SwiftProtobufNamer) -> String {
            ...
            if label == .repeated { return "[\(result)]" }
            return result
      }
      ...
      func swiftStorageType(namer: SwiftProtobufNamer) -> String {
            ...
            switch label {
            case .repeated: return swiftType
            case .optional, .required:
                  if hasFieldPresence { return "\(swiftType)?" }
                  else { return swiftType }
            }
      }
      ...
      func swiftDefaultValue(namer: SwiftProtobufNamer) -> String {
            ...
            case .enum:
                  return namer.dottedRelativeName(
                        enumValue: enumType.defaultValue)
            default: return "0"
            }
      }
      ...
      func traitsType(namer: SwiftProtobufNamer) -> String {
            if isMap {
                  ...
                  case .message:  // Map's can't have a group as the value
                        return
                              "SwiftProtobuf._ProtobufMessageMap<\(keyTraits),\(valueTraits)>"
                  case .enum:
                        return
                              "SwiftProtobuf._ProtobufEnumMap<\(keyTraits),\(valueTraits)>"
                  default:
                        return
                              "SwiftProtobuf._ProtobufMap<\(keyTraits),\(valueTraits)>"
                  }
            }
            switch type {
...
extension EnumDescriptor {
      ...
      func value(named: String) -> EnumValueDescriptor? {
            for v in values { if v.name == named { return v } }
            return nil
      }
}
