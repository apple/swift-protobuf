...
class MessageStorageClassGenerator {
      ...
      init(fields: [FieldGenerator]) { self.fields = fields }

      /// Visibility of the storage within the Message.
      var storageVisibility: String { return "fileprivate" }
      ...
      func generateUniqueStorage(printer p: inout CodePrinter) {
            p.print(
                  "\(storageVisibility) mutating func _uniqueStorage() -> _StorageClass {\n"
            )
            p.indent()
      ...
      private func generateStoredProperties(printer p: inout CodePrinter) {
            for f in fields { f.generateStorage(printer: &p) }
      }
      ...
      private func generateClone(printer p: inout CodePrinter) {
            ...
            p.indent()
            for f in fields { f.generateStorageClassClone(printer: &p) }
            p.outdent()
            p.print("}\n")
      }
}
