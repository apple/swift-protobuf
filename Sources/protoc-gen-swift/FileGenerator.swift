...
class FileGenerator {
      ...
      var outputFilename: String {
            ...
            case .FullPath: return pathParts.dir + pathParts.base + ext
            case .PathToUnderscores:
                  let dirWithUnderscores = pathParts.dir.replacingOccurrences(
                        of: "/", with: "_")
                  return dirWithUnderscores + pathParts.base + ext
            case .DropPath: return pathParts.base + ext
            }
      }
      ...
      init(fileDescriptor: FileDescriptor, generatorOptions: GeneratorOptions) {
            ...
            namer = SwiftProtobufNamer(
                  ...
                  protoFileToModuleMappings:
                        generatorOptions.protoToModuleMappings)
      }
      ...
      func generateOutputFile(
            printer p: inout CodePrinter, errorString: inout String?
      ) {
            p.print(
                  "// DO NOT EDIT.\n", "//\n",
            ...
            // optional.
            let syntaxPath = IndexPath(
                  index: Google_Protobuf_FileDescriptorProto.FieldNumbers.syntax
            )
            if let syntaxLocation = fileDescriptor.sourceCodeInfoLocation(
                  path: syntaxPath)
            {
                  let comments = syntaxLocation.asSourceComment(
                        commentPrefix: "///", leadingDetachedPrefix: "//")
                  if !comments.isEmpty {
                        p.print(comments)
            ...
            if let neededImports
                  = generatorOptions.protoToModuleMappings.neededModules(
                  forFile: fileDescriptor)
            {
                  p.print("\n")
            ...
            let extensionSet = ExtensionSetGenerator(
            ...
            for m in messages {
                  m.generateMainStruct(
                        printer: &p, parent: nil, errorString: &errorString)
            ...
            if !extensionSet.isEmpty {
                  ...
                  extensionSet.generateFileProtobufExtensionRegistry(
                        printer: &p)
                  ...
                  extensionSet.generateProtobufExtensionDeclarations(
                        printer: &p)
            }

            let protoPackage = fileDescriptor.package
            let needsProtoPackage: Bool = !protoPackage.isEmpty
                  && !messages.isEmpty
            if needsProtoPackage || !enums.isEmpty || !messages.isEmpty {
                  ...
                  for m in messages {
                        m.generateRuntimeSupport(
                              printer: &p, file: self, parent: nil)
                  }
            }
      }
