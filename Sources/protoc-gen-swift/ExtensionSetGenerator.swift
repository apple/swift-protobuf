...
class ExtensionSetGenerator {
      ...
      private class ExtensionGenerator {
            ...
            init(
                  ...
                  swiftFullExtensionName = namer.fullName(
                        extensionField: descriptor)

                  comments = descriptor.protoSourceComments()
                  containingTypeSwiftFullName = namer.fullName(
                        message: fieldDescriptor.containingType)
            }

            func generateProtobufExtensionDeclarations(
                  printer p: inout CodePrinter
            ) {
                  let scope = fieldDescriptor.extensionScope == nil
                        ? "" : "static "
                  let traitsType = fieldDescriptor.traitsType(namer: namer)
                  let swiftRelativeExtensionName = namer.relativeName(
                        extensionField: fieldDescriptor)
                  ...
                  if fieldDescriptor.containingType.useMessageSetWireFormat
                        ...
                        === fieldDescriptor.extensionScope
                  { fieldNamePath = fieldDescriptor.messageType.fullName }
                  else { fieldNamePath = fieldDescriptor.fullName }
            ...
            func generateMessageSwiftExtension(printer p: inout CodePrinter) {
                  ...
                  let extensionNames = namer.messagePropertyNames(
                        extensionField: fieldDescriptor)
                  let defaultValue = fieldDescriptor.swiftDefaultValue(
                        namer: namer)
                  ...
                  p.indent()
                  p.print(
                        "return hasExtensionValue(ext: \(swiftFullExtensionName))\n"
                  )
                  p.outdent()
                  p.print("}\n")
                  ...
                  p.indent()
                  p.print(
                        "clearExtensionValue(ext: \(swiftFullExtensionName))\n")
                  p.outdent()
                  p.print("}\n")
            }
      }
      ...
      init(
            fileDescriptor: FileDescriptor, generatorOptions: GeneratorOptions,
      ...
      func add(extensionFields: [FieldDescriptor]) {
            for e in extensionFields {
                  ...
                  let extensionGenerator = ExtensionGenerator(
                        descriptor: e, generatorOptions: generatorOptions,
                        namer: namer)
      ...
      func generateMessageSwiftExtensions(printer p: inout CodePrinter) {
            ...
            let grouped: [ExtensionGenerator] = extensions.enumerated().sorted {
                  ...
                  if $0.element.containingTypeSwiftFullName
                        == $1.element.containingTypeSwiftFullName
                  { return $0.offset < $1.offset }
                  // Otherwise, sort by the Message being extended.
                  return $0.element.containingTypeSwiftFullName
                        < $1.element.containingTypeSwiftFullName
            }.map {
      ...
      func generateFileProtobufExtensionRegistry(printer p: inout CodePrinter) {
            ...
            let filenameAsIdentifer = NamingUtils.toUpperCamelCase(
                  pathParts.base)
      ...
      func generateProtobufExtensionDeclarations(printer p: inout CodePrinter) {
            ...
            for e in extensions {
                  if currentScope !== e.fieldDescriptor.extensionScope {
                        ...
                        let scopeSwiftFullName = namer.fullName(
                              message: currentScope!)
