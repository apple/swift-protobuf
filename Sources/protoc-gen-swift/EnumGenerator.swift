...
class EnumGenerator {
      ...
      init(
            descriptor: EnumDescriptor, generatorOptions: GeneratorOptions,
            ...
            mainEnumValueDescriptors
                  = descriptor.values.filter({ return $0.aliasOf == nil })
            mainEnumValueDescriptorsSorted
                  = mainEnumValueDescriptors.sorted(by: {
      ...
      func generateMainEnum(printer p: inout CodePrinter) {
            ...
            p.print(
                  "\(visibility)enum \(swiftRelativeName): SwiftProtobuf.Enum {\n"
            )
            p.indent()
            ...
            p.indent()
            let dottedDefault = namer.dottedRelativeName(
                  enumValue: enumDescriptor.defaultValue)
      ...
      func generateCaseIterable(
            printer p: inout CodePrinter, includeGuards: Bool = true
      ) {
            ...
            if enumDescriptor.hasUnknownPreservingSemantics {
                  ...
                  p.print(
                        "\(visibility)static var allCases: [\(swiftFullName)] = [\n"
                  )
      ...
      private func generateCasesOrAliases(printer p: inout CodePrinter) {
            let visibility = generatorOptions.visibilitySourceSnippet
            for enumValueDescriptor in namer.uniquelyNamedValues(
                  enum: enumDescriptor)
            {
                  let comments = enumValueDescriptor.protoSourceComments()
                  if !comments.isEmpty { p.print("\n", comments) }
                  let relativeName = namer.relativeName(
                  ...
                  if let aliasOf = enumValueDescriptor.aliasOf {
                        ...
                        p.print(
                              "\(visibility)static let \(relativeName) = \(aliasOfName)\n"
                        )
                  }
                  else {
                        p.print(
                              "case \(relativeName) // = \(enumValueDescriptor.number)\n"
                        )
                  }
            }
            if enumDescriptor.hasUnknownPreservingSemantics {
      ...
      private func generateProtoNameProviding(printer p: inout CodePrinter) {
            ...
            for v in mainEnumValueDescriptorsSorted {
                  if v.aliases.isEmpty {
                        ...
                        let aliasNames
                              = v.aliases.map({ "\"\($0.name)\"" }).joined(
                              separator: ", ")
                        p.print(
                              "\(v.number): .aliased(proto: \"\(v.name)\", aliases: [\(aliasNames)]),\n"
                        )
                  }
            }
            p.outdent()
      ...
      private func generateInitRawValue(printer p: inout CodePrinter) {
            ...
            if enumDescriptor.hasUnknownPreservingSemantics {
                  p.print(
                        "default: self = .\(unrecognizedCaseName)(rawValue)\n")
            }
