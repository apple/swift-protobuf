...
func partition(string: String, atFirstOccurrenceOf substring: String)
      -> (String, String) {
      guard let index = string.range(of: substring)?.lowerBound else {
            ...
            return (
                  ...
                  string.substring(from: string.index(after: index))
            )
      #endif
}
...
func parseParameter(string: String?) -> [(key: String, value: String)] {
      ...
      let asPairs = parts.map {
            partition(string: $0, atFirstOccurrenceOf: "=")
      }
      let result = asPairs.map {
            (key: trimWhitespace($0), value: trimWhitespace($1))
      }
      return result
}
...
func escapedToDataLiteral(_ s: String) -> String {
      if s.isEmpty { return "SwiftProtobuf.Internal.emptyData" }
      var out = "Data(bytes: ["
      var separator = ""
      var escape = false
      var octal = 0
