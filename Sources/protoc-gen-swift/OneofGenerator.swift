...
class OneofGenerator {
      ...
      class MemberFieldGenerator: FieldGeneratorBase, FieldGenerator {
            ...
            var isGroupOrMessage: Bool {
                  ...
                  case .group, .message: return true
      ...
      init(
            descriptor: OneofDescriptor, generatorOptions: GeneratorOptions,
            namer: SwiftProtobufNamer, usesHeapStorage: Bool
      ) {
            ...
            else { storedProperty = "self.\(swiftFieldName)" }
            ...
            var parentNumbers = descriptor.containingType.fields.map {
                  Int($0.number)
            }
            parentNumbers.append(
                  contentsOf: descriptor.containingType.extensionRanges.map {
                        Int($0.start)
                  })
            var parentNumbersIterator
                  = parentNumbers.sorted(by: { $0 < $1 }).makeIterator()
      ...
      func generateMainEnum(printer p: inout CodePrinter) {
            ...
            for f in fields {
                  p.print(
                        "case (\(f.dottedSwiftName)(let l), \(f.dottedSwiftName)(let r)): return l == r\n"
                  )
            }
            if fields.count > 1 {
      ...
      func generateInterface(
            printer p: inout CodePrinter, field: MemberFieldGenerator
      ) {
            // First field causes the oneof enum to get generated.
            if field === fields.first { gerenateOneofEnumProperty(printer: &p) }

            let getter = usesHeapStorage
                  ? "_storage.\(underscoreSwiftFieldName)" : swiftFieldName
            let setter = usesHeapStorage
      ...
      func generateStorage(
            ...
            if usesHeapStorage {
                  p.print(
                        "var \(underscoreSwiftFieldName): \(swiftFullName)?\n")
            }
            else {
      ...
      func generateStorageClassClone(
            printer p: inout CodePrinter, field: MemberFieldGenerator
      ) {
            // First field causes the output.
            guard field === fields.first else { return }
            ...
            p.print(
                  "\(underscoreSwiftFieldName) = source.\(underscoreSwiftFieldName)\n"
            )
      }

      func generateDecodeFieldCase(
            printer p: inout CodePrinter, field: MemberFieldGenerator
      ) {
      ...
      func generateTraverse(
            ...
            if group.count == 1 {
                  p.print(
                        "if case \(field.dottedSwiftName)(let v)? = \(storedProperty) {\n"
                  )
                  p.indent()
                  p.print(
      ...
      func generateFieldComparison(
            ...
            if usesHeapStorage {
                  ...
                  otherStoredProperty
                        = "rhs_storage.\(underscoreSwiftFieldName)"
            }
            else {
            ...
            p.print(
                  "if \(lhsProperty) != \(otherStoredProperty) {return false}\n"
            )
      }

      func generateIsInitializedCheck(
            printer p: inout CodePrinter, field: MemberFieldGenerator
      ) {
            // First field causes the output.
            guard field === fields.first else { return }
            ...
            if fieldsToCheck.count == 1 {
                  ...
                  p.print(
                        "if case \(f.dottedSwiftName)(let v)? = \(storedProperty), !v.isInitialized {return false}\n"
                  )
            }
                  ...
                  for f in fieldsToCheck {
                        p.print(
                              "case \(f.dottedSwiftName)(let v)?: if !v.isInitialized {return false}\n"
                        )
                  }
