...
class MessageGenerator {
      ...
      init(
            descriptor: Descriptor, generatorOptions: GeneratorOptions,
            namer: SwiftProtobufNamer, extensionSet: ExtensionSetGenerator
      ) {
            ...
            // storage yet.
            let useHeapStorage = isAnyMessage || descriptor.fields.count > 16
                  || hasSingleMessageField(descriptor: descriptor)

            oneofs = descriptor.oneofs.map {
                  return OneofGenerator(
                        descriptor: $0, generatorOptions: generatorOptions,
                        namer: namer, usesHeapStorage: useHeapStorage)
            }

            let factory = MessageFieldFactory(
                  generatorOptions: generatorOptions, namer: namer,
                  useHeapStorage: useHeapStorage, oneofGenerators: oneofs)
            ...
            messages = descriptor.messages.filter { return !$0.isMapEntry }.map
            {
                  return MessageGenerator(
                        descriptor: $0, generatorOptions: generatorOptions,
                        namer: namer, extensionSet: extensionSet)
            }
      ...
      func generateMainStruct(
            printer p: inout CodePrinter, parent: MessageGenerator?,
            ...
            if descriptor.useMessageSetWireFormat {
                  guard fields.isEmpty else {
                        errorString
                              = "\(descriptor.fullName) has the option message_set_wire_format but it also has fields."
                        return
                  }
            }
            for e in descriptor.extensions {
                  guard e.containingType.useMessageSetWireFormat else {
                        continue
                  }
            ...
            for f in fields { f.generateInterface(printer: &p) }
            ...
            for o in oneofs { o.generateMainEnum(printer: &p) }

            // Nested enums
            for e in enums { e.generateMainEnum(printer: &p) }

            // Nested messages
            for m in messages {
                  m.generateMainStruct(
                        printer: &p, parent: self, errorString: &errorString)
            }
      ...
      func generateRuntimeSupport(
            printer p: inout CodePrinter, file: FileGenerator,
            ...
            if let parent = parent {
                  ...
                  p.print(
                        "\(visibility)static let protoMessageName: String = \"\(descriptor.name)\"\n"
                  )
            }
            ...
            for e in enums { e.generateRuntimeSupport(printer: &p) }
            for m in messages {
                  m.generateRuntimeSupport(
                        printer: &p, file: file, parent: self)
            }
      }
      ...
      private func generateProtoNameProviding(printer p: inout CodePrinter) {
            if fields.isEmpty {
                  p.print(
                        "\(visibility)static let _protobuf_nameMap = SwiftProtobuf._NameMap()\n"
                  )
            }
            else {
                  p.print(
      ...
      private func generateDecodeMessage(printer p: inout CodePrinter) {
            p.print(
                  "\(visibility)mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {\n"
            )
            p.indent()
            if storage != nil { p.print("_ = _uniqueStorage()\n") }
            ...
            if descriptor.useMessageSetWireFormat && isExtensible {
                  ...
                  let varName: String
                  if fields.isEmpty && !isExtensible { varName = "_" }
                  else { varName = "fieldNumber" }
                  generateWithLifetimeExtension(printer: &p, throws: true) {
                        p in
                        p.print(
                              "while let \(varName) = try decoder.nextFieldNumber() {\n"
                        )
                        p.indent()
                        if !fields.isEmpty {
                              ...
                              if isExtensible {
                                    p.print(
                                          "case \(descriptor.swiftExtensionRangeExpressions):\n"
                                    )
                                    p.indent()
                                    p.print(
                              ...
                              p.print(
                                    "if \(descriptor.swiftExtensionRangeBooleanExpression(variable: "fieldNumber")) {\n"
                              )
                              p.indent()
                              p.print(
                                    "try decoder.decodeExtensionField(values: &_protobuf_extensionFieldValues, messageType: \(swiftFullName).self, fieldNumber: fieldNumber)\n"
      ...
      private func generateTraverse(printer p: inout CodePrinter) {
            p.print(
                  "\(visibility)func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {\n"
            )
            p.indent()
            generateWithLifetimeExtension(printer: &p, throws: true) { p in
                  ...
                  let visitExtensionsName = descriptor.useMessageSetWireFormat
                  ...
                  for f in fieldsSortedByNumber {
                        while nextRange != nil && Int(nextRange!.start)
                              < f.number
                        {
                              p.print(
                                    "try visitor.\(visitExtensionsName)(fields: _protobuf_extensionFieldValues, start: \(nextRange!.start), end: \(nextRange!.end))\n"
                              )
      ...
      private func generateMessageEquality(printer p: inout CodePrinter) {
            ...
            if compareFields {
                  generateWithLifetimeExtension(
                        printer: &p, alsoCapturing: "rhs", selfQualifier: "lhs"
                  ) { p in
                        for f in fields {
                              f.generateFieldComparison(printer: &p)
                        }
                        if storage != nil { p.print("return true\n") }
                  }
            }
            if storage != nil {
      ...
      private func generateIsInitialized(printer p: inout CodePrinter) {
            ...
            if descriptor.file.syntax == .proto2 {
                  ...
                  // fields have values.
                  for f in fields {
                        f.generateRequiredFieldCheck(
                              printer: &fieldCheckPrinter)
                  }
            }
            ...
            if generatedChecks {
                  generateWithLifetimeExtension(printer: &p, returns: true) {
                        p in
      ...
      private func generateWithLifetimeExtension(
            printer p: inout CodePrinter, throws canThrow: Bool = false,
            returns: Bool = false,
            ...
            if storage != nil {
                  ...
                  if let capturedVariable = capturedVariable {
                        ...
                        let actualArgs
                              = "(\(selfQualifier)_storage, \(capturedVariable)._storage)"
                        let formalArgs
                              = "(_args: (_StorageClass, _StorageClass))"
                        p.print(
                        ...
                        p.print(
                              "withExtendedLifetime(\(selfQualifier)_storage) { (_storage: _StorageClass) in\n"
                        )
                        p.indent()
                  }
            }
...
fileprivate struct MessageFieldFactory {
      ...
      init(
            generatorOptions: GeneratorOptions, namer: SwiftProtobufNamer,
            useHeapStorage: Bool, oneofGenerators: [OneofGenerator]
      ) {
            self.generatorOptions = generatorOptions
            self.namer = namer
      ...
      func make(forFieldDescriptor field: FieldDescriptor) -> FieldGenerator {
            if let oneofIndex = field.oneofIndex {
                  return oneofs[Int(oneofIndex)].fieldGenerator(
                        forFieldNumber: Int(field.number))
            }
            else {
                  return MessageFieldGenerator(
                        descriptor: field, generatorOptions: generatorOptions,
                        namer: namer, usesHeapStorage: useHeapStorage)
            }
      }
}
