...
extension Google_Protobuf_Compiler_Version {
      fileprivate var versionString: String {
            if !suffix.isEmpty { return "\(major).\(minor).\(patch).\(suffix)" }
...
struct GeneratorPlugin {
      ...
      func run(args: [String]) -> Int32 {
            ...
            case .generateFromStdin: result = generateFromStdin()
      ...
      private func parseCommandLine(args: [String]) -> Mode {
            ...
            return paths.isEmpty
                  ? .generateFromStdin : .generateFromFiles(paths: paths)
      }
      ...
      private func showVersion() {
            print(
                  "\(CommandLine.programName) \(SwiftProtobuf.Version.versionString)"
            )
      }
      ...
      private func generateFromStdin() -> Int32 {
            ...
            if let dumpPath = ProcessInfo.processInfo.environment[
                  "PROTOC_GEN_SWIFT_LOG_REQUEST"], !dumpPath.isEmpty
            {
                  let dumpURL = URL(fileURLWithPath: dumpPath)
                  do { try requestData.write(to: dumpURL) }
                  catch let e {
                        Stderr.print(
                              "Failed to write request to '\(dumpPath)', \(e)")
                  }
            }
      ...
      private func generateFromFiles(_ paths: [String]) -> Int32 {
            ...
            for p in paths {
                  ...
                  do { requestData = try readFileData(filename: p) }
                  catch let e {
                  ...
                  Stderr.print(
                        "Read request: \(requestData.count) bytes from \(p)")
                  ...
                  if response.hasError {
                        Stderr.print(
                              "Error while generating from \(p) - \(response.error)"
                        )
                        result = 1
                  }
                  else {
      ...
      private func generate(
            ...
            do { options = try GeneratorOptions(parameter: request.parameter) }
                  ...
                  return Google_Protobuf_Compiler_CodeGeneratorResponse(
                        error:
                              "Unknown value for generation parameter '\(name)': '\(value)'"
                  )
            }
            ...
            for name in request.fileToGenerate {
                  let fileDescriptor = descriptorSet.lookupFileDescriptor(
                        protoName: name)
                  let fileGenerator = FileGenerator(
                        fileDescriptor: fileDescriptor,
                        generatorOptions: options)
                  var printer = CodePrinter()
                  fileGenerator.generateOutputFile(
                        printer: &printer, errorString: &errorString)
                  if let errorString = errorString {
                        // If generating multiple files, scope the message with the file that triggered it.
                        let fullError = request.fileToGenerate.count > 1
      ...
      private func auditProtoCVersion(
            request: Google_Protobuf_Compiler_CodeGeneratorRequest
      ) {
      ...
      private func sendReply(
            ...
            do { serializedResponse = try response.serializedData() }
            catch let e {
