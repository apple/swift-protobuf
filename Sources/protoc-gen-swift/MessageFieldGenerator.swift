...
class MessageFieldGenerator: FieldGeneratorBase, FieldGenerator {
      ...
      init(
            descriptor: FieldDescriptor, generatorOptions: GeneratorOptions,
            ...
            if usesHeapStorage {
                  ...
                  storedProperty
                        = "self.\(hasFieldPresence ? underscoreSwiftName : swiftName)"
            }
      ...
      func generateStorage(printer p: inout CodePrinter) {
            ...
            if usesHeapStorage {
                  p.print(
                        "var \(underscoreSwiftName): \(swiftStorageType) = \(defaultValue)\n"
                  )
            }
            else {
                  ...
                  if hasFieldPresence {
                        p.print(
                              "fileprivate var \(underscoreSwiftName): \(swiftStorageType) = \(defaultValue)\n"
                        )
                  }
            }
      }
      ...
      func generateInterface(printer p: inout CodePrinter) {
            ...
            if usesHeapStorage {
                  ...
                  p.indent()
                  let defaultClause = hasFieldPresence
                        ? " ?? \(swiftDefaultValue)" : ""
                  p.print(
                  ...
                  if hasFieldPresence {
                        ...
                        p.print(
                              "\(visibility)var \(swiftName): \(swiftStorageType) = \(swiftDefaultValue)\n"
                        )
                  }
            }
            ...
            let mutableStoragePrefix = usesHeapStorage
                  ? "_uniqueStorage()." : "self."
            p.print(
      ...
      func generateFieldComparison(printer p: inout CodePrinter) {
            ...
            if usesHeapStorage {
                  ...
                  lhsProperty
                        = "lhs.\(hasFieldPresence ? underscoreSwiftName : swiftName)"
                  otherStoredProperty
                        = "rhs.\(hasFieldPresence ? underscoreSwiftName : swiftName)"
            }
      ...
      func generateIsInitializedCheck(printer p: inout CodePrinter) {
            guard isGroupOrMessage
                  && fieldDescriptor.messageType.hasRequiredFields()
            else { return }

            if isRepeated {  // Map or Array
                  p.print(
                        "if !SwiftProtobuf.Internal.areAllInitialized(\(storedProperty)) {return false}\n"
                  )
            }
            else {
                  p.print(
                        "if let v = \(storedProperty), !v.isInitialized {return false}\n"
                  )
            }
      }
      ...
      func generateDecodeFieldCase(printer p: inout CodePrinter) {
            ...
            if isMap {
                  ...
                  decoderMethod
                        = "decode\(modifier)\(fieldDescriptor.protoGenericType)Field"
                  traitsArg = ""
            }

            p.print(
                  "case \(number): try decoder.\(decoderMethod)(\(traitsArg)value: &\(storedProperty))\n"
            )
      }
      ...
      func generateTraverse(printer p: inout CodePrinter) {
            ...
            if isMap {
                  ...
                  let modifier = isPacked ? "Packed" : isRepeated
                        ? "Repeated" : "Singular"
                  visitMethod
                        = "visit\(modifier)\(fieldDescriptor.protoGenericType)Field"
                  traitsArg = ""
            }
            ...
            if isRepeated {  // Also covers maps
                  ...
                  switch fieldDescriptor.type {
                  case .string, .bytes: conditional = ("!\(varName).isEmpty")
                  default: conditional = ("\(varName) != \(swiftDefaultValue)")
                  }
            }
            ...
            p.indent()
            p.print(
                  "try visitor.\(visitMethod)(\(traitsArg)value: \(varName), fieldNumber: \(number))\n"
            )
            p.outdent()
            p.print("}\n")
      }
