...
class GeneratorOptions {
      ...
      init(parameter: String?) throws {
                        ...
                        if !pair.value.isEmpty {
                              do {
                                    ...
                                    throw GenerationError.wrappedError(
                                          message:
                                                "Parameter 'ProtoPathModuleMappings=\(pair.value)'",
                                          error: e)
                              }
                        }
                  case "Visibility":
            ...
            self.protoToModuleMappings = protoFileToModule
                  ?? ProtoFileToModuleMappings()
            self.visibility = visibility

            switch visibility {
            case .Internal: visibilitySourceSnippet = ""
            case .Public: visibilitySourceSnippet = "public "
            }
