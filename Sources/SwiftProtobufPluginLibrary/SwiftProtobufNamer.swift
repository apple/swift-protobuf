...
public final class SwiftProtobufNamer {
      ...
      private func computeRelativeNames(enum e: EnumDescriptor) {
            ...
            for enumValue in e.values {
                  ...
                  else { canidates[canidate] = [enumValue] }
            ...
            for (camelCased, enumValues) in canidates {
                  ...
                  let hasMultipleValues
                        = enumValues.contains(where: {
                        return $0.number != firstValue
                  })

                  guard hasMultipleValues else {
                        // Was the first case, all one value, just aliases that mapped
                        // to the same name.
                        let name = NamingUtils.sanitize(
                              enumCaseName: camelCased)
                        for e in enumValues {
                  ...
                  for e in enumValues {
                        ...
                        // positive.
                        let suffix = e.number >= 0
                              ? "_\(e.number)" : "_n\(-e.number)"
                        enumValueRelativeNameCache[e.fullName]
                              = NamingUtils.sanitize(
                              enumCaseName: camelCased + suffix)
                  }
            }
      }
      ...
      public func fullName(enumValue: EnumValueDescriptor) -> String {
            return fullName(enum: enumValue.enumType) + "." + relativeName(
                  enumValue: enumValue)
      }
      ...
      /// api pov.
      public func uniquelyNamedValues(enum e: EnumDescriptor)
            -> [EnumValueDescriptor]
      {
            return e.values.filter {
                  ...
                  let aliasOfRelativeName = self.relativeName(
                        enumValue: aliasOf)
                  // If the relative name matches for the alias and original, drop
                  // the alias.
                  guard relativeName != aliasOfRelativeName else {
                        return false
                  }
      ...
      public func fullName(oneof: OneofDescriptor) -> String {
            return fullName(message: oneof.containingType) + "." + relativeName(
                  oneof: oneof)
      }
      ...
      public func relativeName(extensionField field: FieldDescriptor) -> String
      {
            ...
            if field.extensionScope != nil {
                  return NamingUtils.sanitize(
                        messageScopedExtensionName: field.namingBase)
            }
            else {
      ...
      public func fullName(extensionField field: FieldDescriptor) -> String {
            ...
            let relativeNameNoBackticks = NamingUtils.trimBackticks(
                  relativeName)
            return extensionScopeSwiftFullName + ".Extensions."
                  + relativeNameNoBackticks
      }

      public typealias MessageFieldNames
            = (name: String, prefixed: String, has: String, clear: String)
      ...
      public func messagePropertyNames(
            ...
            if !includeHasAndClear {
                  return MessageFieldNames(
                        name: fieldName, prefixed: prefixedFieldName, has: "",
                        clear: "")
            }

            let upperName = NamingUtils.toUpperCamelCase(field.namingBase)
            let hasName = NamingUtils.sanitize(
                  fieldName: "has\(upperName)", basedOn: lowerName)
            let clearName = NamingUtils.sanitize(
                  fieldName: "clear\(upperName)", basedOn: lowerName)

            return MessageFieldNames(
                  name: fieldName, prefixed: prefixedFieldName, has: hasName,
                  clear: clearName)
      }
      ...
      public func messagePropertyName(
            oneof: OneofDescriptor, prefixed: String = "_"
      ) -> OneofFieldNames {
      ...
      public typealias MessageExtensionNames
            = (value: String, has: String, clear: String)

      /// Calculate the names to use for the Swift Extension on the extended
      /// message.
      ///
      /// - Precondition: `extensionField` must be FieldDescriptor for an extension.
      public func messagePropertyNames(extensionField field: FieldDescriptor)
            -> MessageExtensionNames
      {
            ...
            if let extensionScope = field.extensionScope {
                  let extensionScopeSwiftFullName = fullName(
                  ...
                  let fieldNameFirstUp = NamingUtils.uppercaseFirstCharacter(
                        fieldName)
                  ...
                  // needed.
                  let swiftPrefix = typePrefix(forFile: field.file)
                  fieldName = NamingUtils.sanitize(
                        fieldName: swiftPrefix + fieldBaseName)
                  if swiftPrefix.isEmpty {
                        ...
                        let upperCleaned = NamingUtils.sanitize(
                              fieldName: NamingUtils.toUpperCamelCase(
                        ...
                        // was capitalized.
                        let fieldNameFirstUp
                              = NamingUtils.uppercaseFirstCharacter(fieldName)
                        hasName = "has" + fieldNameFirstUp
                        clearName = "clear" + fieldNameFirstUp
                  }
            }

            return MessageExtensionNames(
                  value: fieldName, has: hasName, clear: clearName)
      }
      ...
      public func typePrefix(forFile file: FileDescriptor) -> String {
            if let result = filePrefixCache[file.name] { return result }

            let result = NamingUtils.typePrefix(
                  protoPackage: file.package, fileOptions: file.fileOptions)
      ...
      func modulePrefix(file: FileDescriptor) -> String {
            ...
            if prefix == targetModule { return String() }
