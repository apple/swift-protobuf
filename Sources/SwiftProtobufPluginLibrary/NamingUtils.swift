...
///
fileprivate let reservedTypeNames: Set<String> = { () -> Set<String> in
...
fileprivate let reservedFieldNames: Set<String> = { () -> Set<String> in
      var names: Set<String> = []
...
// * backticks around them:
fileprivate let quotableEnumCases: Set<String> = { () -> Set<String> in
      var names: Set<String> = []
...
fileprivate let reservedEnumCases: Set<String> = [
      ...
      "allCases", "debugDescription", "description", "dynamicType", "hashValue",
      "init", "rawValue", "self",
]
...
// * and reserved words.
fileprivate let quotableMessageScopedExtensionNames: Set<String>
      = quotableEnumCases
fileprivate let reservedMessageScopedExtensionNames: Set<String>
      = reservedEnumCases

fileprivate func isAllUnderscore(_ s: String) -> Bool {
      if s.isEmpty { return false }
      for c in s.unicodeScalars { if c != "_" { return false } }
      return true
}

fileprivate func sanitizeTypeName(_ s: String, disambiguator: String) -> String
{
      if reservedTypeNames.contains(s) { return s + disambiguator }
      else if isAllUnderscore(s) { return s + disambiguator }
            ...
            return sanitizeTypeName(truncated, disambiguator: disambiguator)
                  + disambiguator
      }
      else { return s }
}
...
fileprivate func makeUnicodeScalarView(from unicodeScalar: UnicodeScalar)
...
fileprivate func splitIdentifier(_ s: String) -> [String] {
      ...
      for scalar in s.unicodeScalars {
            ...
            if scalar.isDigit {
                  if last.isDigit { current.append(scalar) }
                  else {
                        out.append(current)
                  ...
                  if lastIsUpper { current.append(scalar.lowercased()) }
                  else {
                        out.append(current)
                        current = makeUnicodeScalarView(
                              from: scalar.lowercased())
                  }
            }
            else if isLower {
                  if lastIsLower || lastIsUpper { current.append(scalar) }
                  else {
                        out.append(current)
                        current = makeUnicodeScalarView(from: scalar)
                  }
      ...
      out.append(current)
      if last == "_" { out.append(makeUnicodeScalarView(from: last)) }
...
public enum NamingUtils {
      ...
      static func typePrefix(
            protoPackage: String, fileOptions: Google_Protobuf_FileOptions
      ) -> String {
            // Explicit option (including blank), wins.
            if fileOptions.hasSwiftPrefix { return fileOptions.swiftPrefix }

            if protoPackage.isEmpty { return String() }
      ...
      struct PrefixStripper {
            ...
            init(prefix: String) {
                  self.prefixChars = prefix.lowercased().replacingOccurrences(
            ...
            func strip(from: String) -> String? {
                  ...
                  precondition(
                        fromChars.count == from.lengthOfBytes(using: .ascii))
                  ...
                  let count = fromChars.distance(
                        from: fromChars.startIndex, to: fromIndex)
      ...
      static func sanitize(fieldName s: String, basedOn: String) -> String {
            if basedOn.hasPrefix("clear") && isCharacterUppercase(
                  basedOn, index: 5)
            { return s + "_p" }
            else if basedOn.hasPrefix("has") && isCharacterUppercase(
                  basedOn, index: 3)
            { return s + "_p" }
            else if reservedFieldNames.contains(basedOn) { return s + "_p" }
            ...
            else if isAllUnderscore(basedOn) { return s + "__" }
            else { return s }
      }
      ...
      static func sanitize(enumCaseName s: String) -> String {
            if reservedEnumCases.contains(s) { return "\(s)_" }
            else if quotableEnumCases.contains(s) { return "`\(s)`" }
            else if isAllUnderscore(s) { return s + "__" }
            else { return s }
      }
      ...
      static func uppercaseFirstCharacter(_ s: String) -> String {
            ...
            if let first = out.first {
                  ...
                  result.append(
                        contentsOf: out[
                              out.index(after: out.startIndex)..<out.endIndex])
      ...
      public static func toUpperCamelCase(_ s: String) -> String {
            ...
            for word in t {
                  ...
                  else { out.append(uppercaseFirstCharacter(word)) }
            }
            return out
      }
      ...
      public static func toLowerCamelCase(_ s: String) -> String {
            ...
            var forceLower = true
            for word in t {
                  if forceLower { out.append(word.lowercased()) }
