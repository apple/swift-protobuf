...
extension Google_Protobuf_SourceCodeInfo.Location {
      ...
      public func asSourceComment(
            ...
            func prefixLines(text: String, prefix: String) -> String {
                  ...
                  while !lines.isEmpty && lines.last!.trimmingCharacters(
                        in: .whitespaces).isEmpty
                  { lines.removeLast() }
                  for line in lines { result.append(prefix + line + "\n") }
                  return result
            }
            ...
            if let leadingDetachedPrefix = leadingDetachedPrefix {
                  for detached in leadingDetachedComments {
                        let comment = prefixLines(
                              text: detached, prefix: leadingDetachedPrefix)
                        if !comment.isEmpty {
                              result += comment
            ...
            let comments = hasLeadingComments
                  ? leadingComments : trailingComments
            result += prefixLines(
                  text: escapeMarkup(comments), prefix: commentPrefix)
            return result
      }
}
