...
public struct ProtoFileToModuleMappings {
      ...
      public enum LoadError: Error {
            ...
            case duplicateProtoPathMapping(
                  path: String, firstModule: String, secondModule: String
            )
      }
      ...
      public init(path: String) throws {
            ...
            catch { throw LoadError.failToOpen(path: path) }

            let mappingsProto = try SwiftProtobuf_GenSwift_ModuleMappings(
                  textFormatString: content)
      ...
      public init(
            ...
            for (idx, mapping) in mappings.mapping.lazy.enumerated() {
                  if mapping.moduleName.isEmpty {
                        throw LoadError.entryMissingModuleName(
                              mappingIndex: idx)
                  }
                        ...
                        if let existing = builder[path] {
                              if existing != mapping.moduleName {
                                    throw LoadError.duplicateProtoPathMapping(
                                          path: path, firstModule: existing,
                                          secondModule: mapping.moduleName)
                              }// Was a repeat, just allow it.
                        }
                        else { builder[path] = mapping.moduleName }
                  }
            }
            self.mappings = builder
      }

      public init() { mappings = wktMappings }
      ...
      public func neededModules(forFile file: FileDescriptor) -> [String]? {
            if file.dependencies.isEmpty { return nil }
            ...
            if collector.isEmpty { return nil }
