...
struct SwiftProtobufPlugin {
    ...
    enum PluginError: Error {
        ...
        /// Indicates that the input directory had a trailing `/`.
        case invalidDirectoryFormat
        /// Indicates that there was no configuration file at the required location.
        case noConfigFound
        /// Indicates that there was no file or directory mentioned in the json.
        case noProtoSourceFound
    }
    ...
    struct Configuration: Codable {
        ...
        struct Invocation: Codable {
            ...
            /// An array of directories that contains `.proto` files for this invocation.
            var protoDirectories: [String]?
            /// The visibility of the generated files.
    ...
    ///   - currentWorkingDirectory: The absolute path of the current working directory.
    ...
    func createBuildCommands(
        ...
        currentWorkingDirectory: PackagePlugin.Path
    ) throws -> [Command] {
    ...
    private func invokeProtoc(
        ...
        func addProtoFileToIO(file: String) {
            var file = file
        ...
        for file in invocation.protoFiles ?? [] {
            addProtoFileToIO(file: file)
        }
        
        for protoDirectory in invocation.protoDirectories ?? [] {
            let protoFiles = try getProtoFiles(
                from: protoDirectory,
                in: directory.string.replacingOccurrences(
                    of: currentWorkingDirectory.string.appending("/"),
                    with: ""
                )
            )
    ...
    /// Matches files from the given path and parent directory. If the given path is a file, it will return the file immediately.
    /// - Parameters:
    ///   - path: The file path that you want to search. Paths should be terminated with `/`.
    ///   - parent: The parent directory of the path.
    /// - Returns: An array of proto files from the directory and all its subdirectories.
    private func getProtoFiles(
        from path: String,
        in parent: String
    ) throws -> [String] {
        let url = URL(fileURLWithPath: parent.appending("/\(path)"))
        var files = [String]()
            ...
            if let enumerator = FileManager.default.enumerator(
                ...
                for case let fileURL as URL in enumerator {
                    ...
                    if fileAttributes.isRegularFile == true,
                       fileURL.pathExtension == "proto" {
                        if !path.isEmpty {
                            files.append("\(path)/\(fileURL.relativeString)")
                        } else {
                            files.append("\(fileURL.relativeString)")
                        }
                    }
                }
            }
    ...
    private func validateConfiguration(_ configuration: Configuration) throws {
        for invocation in configuration.invocations {
            if invocation.protoFiles?.isEmpty == true &&
                invocation.protoDirectories?.isEmpty == true {
                throw PluginError.noProtoSourceFound
            }
            ...
            for protoDirectory in invocation.protoDirectories ?? [] {
                if protoDirectory.hasSuffix("/") {
                    throw PluginError.invalidDirectoryFormat
                }
            }
        }
    }
...
extension SwiftProtobufPlugin: BuildToolPlugin {
    func createBuildCommands(
        ...
        return try createBuildCommands(
            ...
            tool: context.tool,
            currentWorkingDirectory: context.package.directory
        )
    }
}
...
extension SwiftProtobufPlugin: XcodeBuildToolPlugin {
    func createBuildCommands(
        ...
        return try createBuildCommands(
            ...
            tool: context.tool,
            currentWorkingDirectory: context.xcodeProject.directory
        )
    }
}
#endif
