...
class Test_NamingUtils: XCTestCase {
  ...
  func testSanitize_messageName() {
    ...
    for (input, expected) in tests {
      XCTAssertEqual(
        NamingUtils.sanitize(messageName: input, forbiddenTypeNames: ["RenamedSwiftProtobuf"]),
        expected)
    }
  }
  ...
  func testSanitize_enumName() {
    ...
    for (input, expected) in tests {
      XCTAssertEqual(
        NamingUtils.sanitize(enumName: input, forbiddenTypeNames: ["RenamedSwiftProtobuf"]),
        expected)
    }
  }
  ...
  func testSanitize_oneofName() {
    ...
    for (input, expected) in tests {
      XCTAssertEqual(
        NamingUtils.sanitize(oneofName: input, forbiddenTypeNames: ["RenamedSwiftProtobuf"]),
        expected)
    }
  }
  ...
  func testSanitize_fieldName() {
    ...
    for (input, expected) in tests {
      ...
      let expected2 =
        "XX" + NamingUtils.uppercaseFirstCharacter(NamingUtils.trimBackticks(expected))
