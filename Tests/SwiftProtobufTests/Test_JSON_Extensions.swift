...
class Test_JSON_Extensions: XCTestCase, PBTestHelpers {
  ...
  func test_optionalInt32Extension() throws {
    ...
    assertJSONDecodeFails(
      "{\"[protobuf_unittest.UNKNOWN_EXTENSION]\":17}",
      extensions: extensions)
    assertJSONDecodeFails(
  ...
  func test_optionalMessageExtension() throws {
    assertJSONEncode(
      "{\"[protobuf_unittest.optional_nested_message_extension]\":{\"bb\":12}}",
      extensions: extensions
    ) {
      (o: inout MessageTestType) in
  ...
  func test_repeatedInt32Extension() throws {
    assertJSONEncode(
      "{\"[protobuf_unittest.repeated_int32_extension]\":[1,2,3,17]}",
      extensions: extensions
    ) {
      (o: inout MessageTestType) in
  ...
  func test_repeatedMessageExtension() throws {
    assertJSONEncode(
      "{\"[protobuf_unittest.repeated_nested_message_extension]\":[{\"bb\":12},{}]}",
      extensions: extensions
    ) {
      (o: inout MessageTestType) in
  ...
  func test_optionalStringExtensionWithDefault() throws {
    assertJSONEncode(
      "{\"[protobuf_unittest.default_string_extension]\":\"hi\"}", extensions: extensions
    ) {
      (o: inout MessageTestType) in
...
class Test_JSON_RecursiveNested_Extensions: XCTestCase, PBTestHelpers {
  ...
  func test_nestedMessage() throws {
    assertJSONEncode(
      "{\"[protobuf_unittest.extend.a_b]\":12}",
      extensions: extensions
    ) {
      (o: inout MessageTestType) in
    ...
    assertJSONDecodeSucceeds(
      "{\"[protobuf_unittest.extend.m2]\":{\"[protobuf_unittest.extend.aB]\":23}}",
      extensions: extensions
    ) {
