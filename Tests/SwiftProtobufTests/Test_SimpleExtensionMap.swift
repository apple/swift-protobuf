...
extension AnyMessageExtension {
  ...
  func isEqual(_ other: AnyMessageExtension) -> Bool {
    return
      (fieldNumber == other.fieldNumber && fieldName == other.fieldName
...
let ext1 = MessageExtension<
  OptionalExtensionField<ProtobufInt32>, ProtobufUnittest_TestAllExtensions
>(
  _protobuf_fieldNumber: 1,
  fieldName: "my_ext1"
)

let ext2 = MessageExtension<
  OptionalExtensionField<ProtobufInt64>, ProtobufUnittest_TestAllExtensions
>(
...
let ext3 = MessageExtension<
  OptionalExtensionField<ProtobufDouble>, ProtobufUnittest_TestPackedExtensions
>(
  _protobuf_fieldNumber: 1,
  fieldName: "my_ext1b"
)
...
let ext4 = MessageExtension<
  OptionalExtensionField<ProtobufBool>, ProtobufUnittest_TestAllExtensions
>(
  _protobuf_fieldNumber: 2,
  fieldName: "my_ext4"
)
...
class Test_SimpleExtensionMap: XCTestCase {
  ...
  func testFieldNumberForProto() {
    ...
    let lookup1 = map1.fieldNumberForProto(
      messageType: ext1.messageType,
      protoFieldName: ext1.fieldName)
    XCTAssertEqual(lookup1, ext1.fieldNumber)

    let lookup2 = map1.fieldNumberForProto(
      messageType: ext2.messageType,
      protoFieldName: ext2.fieldName)
    XCTAssertEqual(lookup2, ext2.fieldNumber)

    let lookup3 = map1.fieldNumberForProto(
      messageType: ext1.messageType,
      protoFieldName: "foo_bar_baz")
    XCTAssertNil(lookup3)

    let lookup4 = map1.fieldNumberForProto(
      messageType: ProtobufUnittest_TestAllTypes.self,
      protoFieldName: ext1.fieldName)
    XCTAssertNil(lookup4)
    ...
    let lookup1b = map2.fieldNumberForProto(
      messageType: ext1.messageType,
      protoFieldName: ext1.fieldName)
    XCTAssertEqual(lookup1b, ext1.fieldNumber)

    let lookup2b = map2.fieldNumberForProto(
      messageType: ext2.messageType,
      protoFieldName: ext2.fieldName)
    XCTAssertNil(lookup2b)

    let lookup3b = map2.fieldNumberForProto(
      messageType: ext4.messageType,
