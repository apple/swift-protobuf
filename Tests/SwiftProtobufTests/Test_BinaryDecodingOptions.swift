...
class Test_BinaryDecodingOptions: XCTestCase {
    ...
    for (i, (binaryInput, messageType, extensions, testCases)) in tests.enumerated() {
      for (limit, expectSuccess) in testCases {
        do {
          ...
          let _ =
            try messageType.init(
              serializedData: Data(binaryInput),
          ...
          if expectSuccess {
            XCTFail(
              "Decode failed because of limit, but should *NOT* have, pass: \(i), limit: \(limit)")
          } else {
  ...
  func testDiscaringUnknownFields() throws {
    ...
    do {
      ...
      let msg2 = try ProtobufUnittest_TestEmptyMessage(
    ...
    do {
      ...
      let msg2 = try ProtobufUnittest_TestAllTypes(
    ...
    do {
      ...
      XCTAssertTrue(msg1.unknownFields.data.isEmpty)
      XCTAssertEqual(
        msg1.ProtobufUnittest_optionalNestedMessageExtension.unknownFields.data,
    ...
    do {
      ...
      XCTAssertTrue(msg1.unknownFields.data.isEmpty)
      XCTAssertEqual(
        msg1.ProtobufUnittest_optionalGroupExtension.unknownFields.data,
        inputCurrentLevelData)
    ...
    do {
      ...
      let msg2 = try ProtobufUnittest_TestAllTypes(
        serializedData: inputUnknownEnumData,
        options: discardOptions)
      XCTAssertTrue(msg2.unknownFields.data.isEmpty)
    }
  }
