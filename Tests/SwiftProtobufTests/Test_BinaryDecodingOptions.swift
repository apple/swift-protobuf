...
class Test_BinaryDecodingOptions: XCTestCase {
    ...
    func testDiscardingUnknownFields() throws {
        ...
        do {
            let msg1 = try ProtobufUnittest_TestEmptyMessage(contiguousBytes: inputCurrentLevel)
            XCTAssertEqual(Array(msg1.unknownFields.data), inputCurrentLevel)

            let msg2 = try ProtobufUnittest_TestEmptyMessage(contiguousBytes: inputCurrentLevel,
        ...
        do {
            let msg1 = try ProtobufUnittest_TestAllTypes(contiguousBytes: inputSubMessage)
            ...
            let msg2 = try ProtobufUnittest_TestAllTypes(contiguousBytes: inputSubMessage,
                                                          options: discardOptions)
            XCTAssertTrue(msg2.unknownFields.data.isEmpty)
        ...
        //   }
        do {
            let msg1 = try ProtobufUnittest_TestAllTypes(contiguousBytes: inputGroup)
            XCTAssertTrue(msg1.unknownFields.data.isEmpty)
            XCTAssertEqual(Array(msg1.optionalGroup.unknownFields.data), inputCurrentLevel)

            let msg2 = try ProtobufUnittest_TestAllTypes(contiguousBytes: inputGroup,
                                                          options: discardOptions)
            XCTAssertTrue(msg2.unknownFields.data.isEmpty)
        ...
        do {
            let msg1 = try ProtobufUnittest_TestAllTypes(contiguousBytes: inputUnknownEnum)
            XCTAssertEqual(Array(msg1.unknownFields.data), inputUnknownEnum)

            let msg2 = try ProtobufUnittest_TestAllTypes(contiguousBytes: inputUnknownEnum,
                                                          options: discardOptions)
            XCTAssertTrue(msg2.unknownFields.data.isEmpty)
        }
    }
