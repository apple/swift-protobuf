...
class Test_Enum_Proto2: XCTestCase, PBTestHelpers {
  ...
  func testEqual() {
    XCTAssertEqual(
      ProtobufUnittest_TestEnumWithDupValue.foo1, ProtobufUnittest_TestEnumWithDupValue.foo2)
    XCTAssertNotEqual(
      ProtobufUnittest_TestEnumWithDupValue.foo1, ProtobufUnittest_TestEnumWithDupValue.bar1)
  }
  ...
  func testUnknownValues() throws {
    ...
    let msgPrime = try Proto3PreserveUnknownEnumUnittest_MyMessagePlusExtra(
      serializedData: msgSerialized)
  ...
  func testEnumPrefixStripping_JSON() throws {
    ...
    json =
      "{\"values4\":[\"ENUM_TEST_RESERVED_WORD_VAR\",\"ENUM_TEST_RESERVED_WORD_NOT_RESERVED\"]}"
    msg = ProtobufUnittest_SwiftEnumTest.with {
      $0.values4 = [.var, .notReserved]
    }
    XCTAssertEqual(try msg.jsonString(), json)
