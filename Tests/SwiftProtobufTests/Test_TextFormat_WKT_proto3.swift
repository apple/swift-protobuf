...
class Test_TextFormat_WKT_proto3: XCTestCase, PBTestHelpers {
  ...
  func assertAnyTest<M: Message & Equatable>(
    _ message: M, expected: String, file: XCTestFileArgType = #file, line: UInt = #line
  ) {
    ...
    do {
      ...
      XCTAssert(
        r, "Encode/decode cycle should generate equal object: \(decoded) != \(configured)",
        file: file, line: line)
    } catch {
      XCTFail(
        "Encode/decode cycle should not throw error, decoding: \(error)", file: file, line: line)
    }
  }
    ...
    let a = try ProtobufUnittest_TestWellKnownTypes.with {
      $0.anyField = try Google_Protobuf_Any(
        message: Google_Protobuf_Any(
          message: Google_Protobuf_Duration(seconds: 123, nanos: 234_567_890)))
    }
  ...
  func testAny_verbose() {
    ...
    do {
      a = try ProtobufUnittest_TestWellKnownTypes(
        textFormatString:
    ...
    do {
      b = try ProtobufUnittest_TestWellKnownTypes(
        textFormatString:
