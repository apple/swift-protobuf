...
class Test_JSONEncodingOptions: XCTestCase {
  ...
  func testAlwaysPrintEnumsAsInts() {
    ...
    XCTAssertEqual(
      try ProtobufUnittest_Message3.jsonString(from: msgArray, options: asStrings),
      "[" + "{\"optionalEnum\":\"BAR\"}" + "," + "{\"repeatedEnum\":[\"BAR\",\"BAZ\"]}" + ","
        + "{\"mapInt32Enum\":{\"42\":\"BAZ\"}}" + "]")
    XCTAssertEqual(
      try ProtobufUnittest_Message3.jsonString(from: msgArray, options: asInts),
      "[" + "{\"optionalEnum\":1}" + "," + "{\"repeatedEnum\":[1,2]}" + ","
        + "{\"mapInt32Enum\":{\"42\":2}}" + "]")
  ...
  func testPreserveProtoFieldNames() {
    ...
    XCTAssertEqual(
      try msg2.jsonString(options: protoNames), "{\"repeated_enum\":[\"BAR\",\"BAZ\"]}")
    ...
    XCTAssertEqual(
      try msg3.jsonString(options: protoNames), "{\"map_int32_enum\":{\"42\":\"BAZ\"}}")
    ...
    let msgArray = [msg1, msg2, msg3]
    XCTAssertEqual(
      try ProtobufUnittest_Message3.jsonString(from: msgArray, options: jsonNames),
      "[" + "{\"optionalEnum\":\"BAR\"}" + "," + "{\"repeatedEnum\":[\"BAR\",\"BAZ\"]}" + ","
        + "{\"mapInt32Enum\":{\"42\":\"BAZ\"}}" + "]")
    XCTAssertEqual(
      try ProtobufUnittest_Message3.jsonString(from: msgArray, options: protoNames),
