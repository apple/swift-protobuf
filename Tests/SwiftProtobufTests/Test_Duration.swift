...
class Test_Duration: XCTestCase, PBTestHelpers {
  ...
  func testJSON_durationField() throws {
    do {
      let valid = try ProtobufTestMessages_Proto3_TestAllTypesProto3(
        jsonString: "{\"optionalDuration\": \"1.001s\"}")
    ...
    XCTAssertThrowsError(
      try ProtobufTestMessages_Proto3_TestAllTypesProto3(
        jsonString: "{\"optionalDuration\": \"1.001\"}"))
  }
  ...
  func testTranscode() throws {
    ...
    XCTAssertEqual(
      try parsedMax.serializedData(),
      Data([234, 18, 13, 8, 128, 188, 174, 206, 151, 9, 16, 255, 147, 235, 220, 3]))
    ...
    XCTAssertEqual(
      try parsedMin.serializedData(),
      Data([
        234, 18, 22, 8, 128, 196, 209, 177, 232, 246, 255, 255, 255, 1, 16, 129, 236, 148, 163, 252,
        255, 255, 255, 255, 1,
      ]))
  }

  func testConformance() throws {
    let tooSmall = try ProtobufTestMessages_Proto3_TestAllTypesProto3(
      serializedData: Data([234, 18, 11, 8, 255, 195, 209, 177, 232, 246, 255, 255, 255, 1]))
    ...
    let tooBig = try ProtobufTestMessages_Proto3_TestAllTypesProto3(
      serializedData: Data([234, 18, 7, 8, 129, 188, 174, 206, 151, 9]))
