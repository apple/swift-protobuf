...
class Test_Extensions: XCTestCase, PBTestHelpers {
  ...
  func assertEncode(
    ...
    do {
      ...
      XCTAssert(
        Data(expected) == encoded, "Did not encode correctly: got \(encoded)", file: file,
        line: line)
      do {
  ...
  func test_optionalInt32Extension() throws {
    ...
    let m1 = try ProtobufUnittest_TestAllExtensions(
      serializedData: Data([8, 17]), extensions: extensions)
    ...
    XCTAssertEqual(
      m2.debugDescription,
      "SwiftProtobufTests.ProtobufUnittest_TestAllExtensions:\n[protobuf_unittest.optional_int32_extension]: 18\n"
    )
  ...
  func test_repeatedInt32Extension() throws {
    ...
    assertDecodeSucceeds([248, 1, 7, 248, 1, 8]) {
      $0.ProtobufUnittest_repeatedInt32Extension == [7, 8]
    }
    assertDecodeSucceeds([250, 1, 2, 7, 8]) {
      $0.ProtobufUnittest_repeatedInt32Extension == [7, 8]
    }
